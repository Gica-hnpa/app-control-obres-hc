...
function LoginScreen8778({onLogin}){
  ...
  function submit(e){
    ...
    if(APP_USERS8779[u]&&APP_USERS8779[u]===pwd){sessionStorage.setItem("aco_current_user8779",u);onLogin?.(u);return}
  ...
  return <div className="login-page-v8778"><form className="login-card-v8778" onSubmit={submit}>
    ...
    <label><span>Usuari</span><input autoFocus value={user} onChange={e=>setUser(e.target.value)} placeholder="Usuari" autoComplete="username"/></label>
    <label><span>Contrasenya</span><input type="password" value={pwd} onChange={e=>setPwd(e.target.value)} placeholder="Contrasenya" autoComplete="current-password"/></label>
    {err&&<div className="login-error-v8778">{err}</div>}
    <button className="primary">Entrar</button><small>Les dades queden separades per usuari en aquest dispositiu. L’usuari de prova extern entra buit.</small>
  </form></div>
}
...
useEffect(()=>{
  ...
  c=(c||[]).map(x=>x.id==="socoterm"?{...x,logo:x.logo||SOCOTERM_LOGO}:x);
  setClients(c);setObres(o);setOdata(d);
  setClientId(c[0]?.id||"");setObraId(o[0]?.id||"");setTab("Resum");setScreen("Inici");
},[authUser8779]);
useEffect(()=>{if(authUser8779)lsSet8779("aco_clients",JSON.stringify(clients),authUser8779)},[clients,authUser8779]);
useEffect(()=>{if(authUser8779)lsSet8779("aco_obres",JSON.stringify(obres),authUser8779)},[obres,authUser8779]);
useEffect(()=>{if(authUser8779)lsSet8779("aco_odata",JSON.stringify(odata),authUser8779)},[odata,authUser8779]);
useEffect(()=>{if(authUser8779&&(obres||[]).some(o=>!o.codiExpedient||!o.expedientBase)){setObres(p=>assignMissingCodes8739(p,clients))}},[authUser8779]);
const obra=obres.find(o=>o.id===obraId)||obres[0]||{id:"",client:"",nom:"Sense expedient",propietat:"Client pendent",nifPropietat:"Pendent",adreca:"",poblacio:"",tipusTreball:"Altres",tipologia:"Altres",estat:"Pendent",any:String(new Date().getFullYear())}, client=clients.find(c=>c.id===obra?.client)||{id:"",nom:obra?.propietat||"Client pendent",rao:obra?.propietat||"Client pendent",nif:obra?.nifPropietat||"Pendent",email:"Pendent",telefon:"Pendent",adreca:obra?.adreca||"Pendent",logo:""}, data=obra?.id?(odata[obra.id]||empty()):empty();
