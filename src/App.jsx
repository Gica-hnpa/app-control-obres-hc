...
function printQuote8745(type,doc,obra){
  ...
  const html=`<!doctype html><html><head><meta charset="utf-8"><title>${esc(title)} ${esc(doc.numero||"")}</title><style>@page{size:A4;margin:14mm}body{font-family:Arial,Helvetica,sans-serif;color:#111827;font-size:12px}.top{display:flex;justify-content:space-between;border-bottom:2px solid #111827;padding-bottom:10px;margin-bottom:14px}h1{margin:0;font-size:24px}.parties{display:grid;grid-template-columns:1fr 1fr;gap:10mm;margin-bottom:12px}.box{border:1px solid #cbd5e1;border-radius:6px;padding:9px;min-height:30mm}.box h3{margin:0 0 6px;font-size:11px;color:#64748b;text-transform:uppercase}.box b,.box span{display:block;margin-top:3px}.exp{border:1px solid #cbd5e1;background:#f8fafc;border-radius:6px;padding:9px;margin-bottom:12px}table{width:100%;border-collapse:collapse;table-layout:fixed}th,td{border:1px solid #cbd5e1;padding:8px;vertical-align:top}th{background:#f1f5f9;text-align:left}.c1{width:62%}.c2{width:14%}.c3{width:10%}.c4{width:14%}.num{text-align:right;white-space:nowrap}p{white-space:pre-wrap;line-height:1.45;color:#334155}.totals{width:75mm;margin-left:auto;margin-top:16px}.totals div{display:flex;justify-content:space-between;border-bottom:1px solid #e5e7eb;padding:6px 0}.totals .total{border-top:2px solid #111827;border-bottom:0;font-size:18px;margin-top:5px;padding-top:10px}.foot{margin-top:28px;border-top:1px solid #e5e7eb;padding-top:8px;color:#64748b;font-size:11px}</style></head><body><div class="top"><h1>${esc(title)}</h1><b>${esc(doc.numero||"—")}</b></div><div class="parties"><div class="box"><h3>Dades del tècnic</h3><b>${esc(cfg.empresa||"Héctor Cubero / Despatx tècnic")}</b><span>${esc(cfg.email||"Email pendent")}</span><span>NIF / Col·legiat: pendent</span></div><div class="box"><h3>Client</h3><b>${esc(obra?.propietat||"Client")}</b><span>NIF: ${esc(obra?.nifPropietat||"Pendent")}</span><span>${esc(obra?.adreca||"")}</span><span>${esc(obra?.poblacio||"")}</span></div></div><div class="exp"><b>Expedient</b><br>${esc(expedientCode8739(obra))} · ${esc(obra?.nom||"")}<br><small>Data: ${esc(doc.data||"—")}</small></div><table><colgroup><col class="c1"><col class="c2"><col class="c3"><col class="c4"></colgroup><thead><tr><th>Concepte</th><th>Base imposable</th><th>IVA</th><th>Total</th></tr></thead><tbody><tr><td><b>${esc(doc.concepte||"Honoraris tècnics")}</b><p>${esc(doc.text||"—")}</p></td><td class="num">${esc(money(base))}</td><td class="num">${esc(doc.iva||21)}%<br><small>${esc(money(iva))}</small></td><td class="num"><b>${esc(money(total))}</b></td></tr></tbody></table><div class="totals"><div><span>Base imposable</span><b>${esc(money(base))}</b></div><div><span>IVA</span><b>${esc(money(iva))}</b></div><div class="total"><span>Total</span><b>${esc(money(total))}</b></div></div><div class="foot">Document provisional pendent d’adaptar a dades fiscals definitives del tècnic/despatx.</div><script>window.onload=()=>setTimeout(()=>window.print(),300)</script></body></html>`;
...
function addObra(e){e.preventDefault();let f=new FormData(e.currentTarget),id="obra-"+Date.now();let clientId=f.get("client");let nouClient=null;
if(clientId==="__new__"||!clientId){
  clientId="client-"+Date.now();
  let nom=f.get("clientNouNom")||"Nou client";
  nouClient={id:clientId,nom,rao:f.get("clientNouRao")||nom,tipus:f.get("clientNouTipus")||"Particular",contacte:f.get("clientNouContacte")||"Pendent",nif:f.get("clientNouNif")||"Pendent",email:f.get("clientNouEmail")||"Pendent",telefon:f.get("clientNouTelefon")||"Pendent",adreca:f.get("clientNouAdreca")||"Pendent",color:"blue",logo:""};
...
let built=buildExpedientCode8739({year,number:nextExpNumber8739(year,obres),tipus:tipusTreball,client:clientObj,clientNom:f.get("clientNouNom"),keyword,nom:f.get("nom"),subtitol:f.get("subtitol"),poblacio:f.get("poblacio")});
let o={id,client:clientId,any:year,nom:f.get("nom")||"Nou expedient",subtitol:f.get("subtitol")||tipusTreball,tipologia:tipusTreball,tipusTreball,modul:"tecnic",estat:f.get("estat")||"Pressupostada",pressupost:0,certificacio:0,propietat:f.get("propietat")||f.get("clientNouNom")||"Pendent",nifPropietat:f.get("nifPropietat")||f.get("clientNouNif")||"Pendent",adreca:f.get("adreca")||"Pendent",poblacio:f.get("poblacio")||"Pendent",rc:f.get("rc")||"Pendent",imatge:"",properaCert:"Pendent",properaVisita:"Pendent",...built,codiExpedient:built.codi};
setObres(p=>[o,...p]);setD(id,empty());setModal(null);openObra(id)}
...
latest.push(...pressupostos.map(p=>({tipus:"Pressupost",data:p.data||"—",txt:p.nom||p.versio||"Pressupost tècnic"})));
latest.push(...factures.map(f=>({tipus:"Factura",data:f.data||"—",txt:f.numero||f.tipus||"Factura / proforma"})));
...
return <div className="stack resum-tecnic-v8738">
  <section className="resum-hero-v8738">
    <div>
      <span className="eyebrow-v8735">Fitxa resum de l’expedient</span>
      <h2>{obra?.nom||"Expedient"}</h2>
      <p>{client?.nom||obra?.propietat||"Client"} · {moduleLabel8737(obra)}</p>
    </div>
    <div className="status-pill-v8738"><small>Estat</small><b>{obra?.estat||"Pendent"}</b></div>
  </section>
  <section className="resum-kpi-grid-v8738">
    <div className="resum-kpi-pro-v8735 main"><span>Tipus de treball</span><b>{moduleLabel8737(obra)}</b><em>Mòdul 1 · Tècnic</em></div>
    <div className="resum-kpi-pro-v8735"><span>Proper avís / cita</span><b>{proper?`${proper.day}/${String((proper.month??0)+1).padStart(2,"0")}`:"—"}</b><em>{proper?`${proper.type||"Agenda"} · ${proper.title||proper.note||""}`:"Sense avisos programats"}</em></div>
    <div className="resum-kpi-pro-v8735 ok"><span>Temps registrat</span><b>{totalHores.toFixed(2)} h</b><em>{money(costTemps)} valor intern</em></div>
    <div className="resum-kpi-pro-v8735 warn"><span>Documents / fotos</span><b>{docs.length+fotos.length}</b><em>{docs.length} docs · {fotos.length} fotos</em></div>
  </section>
  <section className="resum-info-grid-v8738">
    <Card title="Darreres actuacions i avisos de l’expedient">
      <div className="activity-list-v8738">
        {latest.length===0&&<div className="activity-empty-v8738"><b>Sense activitat recent</b><span>Quan afegeixis actes, documents, cites, pressupostos, factures o temps de treball apareixeran aquí.</span></div>}
        {latest.map((x,i)=><div className="activity-row-v8738" key={i}><strong>{x.tipus}</strong><span>{x.txt}</span><em>{x.data}</em></div>)}
      </div>
    </Card>
    <Card title="Dades bàsiques de control">
      <div className="obra-data-mini-v8735">
        <p><b>Client final:</b> {obra?.propietat||"—"}</p>
...
function QuotePreview8743({type="pressupost",doc,obra,close}){
  ...
  return <Modal title={`Vista prèvia ${isFactura?"factura":"pressupost"}`} close={close}>
    <div className="a4-preview-wrap-v8743">
      <div className="quote-a4-v8743 quote-a4-v8744 print-area">
        ...
        <table className="quote-a4-table-v8743 quote-a4-table-v8745"><colgroup><col className="col-concepte"/><col className="col-base"/><col className="col-iva"/><col className="col-total"/></colgroup><thead><tr><th>Concepte</th><th>Base imposable</th><th>IVA</th><th>Total</th></tr></thead><tbody><tr><td><b>{doc.concepte||"Honoraris tècnics"}</b><p>{doc.text||"—"}</p></td><td className="numcell">{money(base)}</td><td className="numcell iva-cell-v8745">{doc.iva||21}%<small>{money(iva)}</small></td><td className="numcell"><b>{money(total)}</b></td></tr></tbody></table>
        ...
        <div className="quote-a4-footer-v8734">Document provisional pendent d’adaptar a dades fiscals definitives del tècnic/despatx.</div>
      </div>
    </div>
...
function InlineQuotePreview8744({type="pressupost",doc,obra}){
  const isFactura=type==="factura";
  const total=totalIva8743(doc);
  return <div className="inline-preview-v8744"><div className="inline-a4-v8744"><div><b>{isFactura?"Factura / proforma":"Pressupost"}</b><span>{doc.concepte||"Honoraris tècnics"}</span></div><p>{doc.text||"Text pendent"}</p><strong>Total IVA inclòs: {money(total)}</strong><small>{expedientCode8739(obra)} · {obra?.propietat||"Client"}</small></div></div>
}
function PressupostTecnic8738({data,obra,addPressupost,updatePressupost,facturarPressupost,deletePressupost,openEmail,openDoc}){
  ...
  return <div className="stack quote-module-v8742 quote-module-v8743 quote-module-v8744">{preview&&<QuotePreview8743 type="pressupost" doc={preview} obra={obra} close={()=>setPreview(null)}/>}<Card title="Pressupostos" action={<button className="primary" onClick={()=>{setOpen(true);setEditing(null)}}><Plus/> Nou pressupost</button>}>
    ...
    {open&&<form className="quote-form-v8742 quote-form-v8744" onSubmit={submit}>
      ...
      <label className="span-all quote-text-v8742 quote-text-v8744"><span>Text del pressupost</span><textarea value={text} onChange={e=>setText(e.target.value)}/></label>
