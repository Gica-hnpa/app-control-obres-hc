...
function stopTimer(){let real=Math.max(timer.elapsed/3600000,0);let h=roundQuarterHour878163(real);setD(obraId,d=>({...d,hores:[...d.hores,{id:"h"+Date.now(),data:new Date().toLocaleDateString("ca-ES"),etiqueta:timer.label,tasca:timer.task||"Temps cronometrat",inici:"—",final:"—",hores:h,horesReals:real,preu:+timer.rate||50,observacions:`Temps real ${formatTimer878163(real)} · facturat en fraccions de 15 min`}]}));setTimer(t=>({...t,running:false,start:null,elapsed:0,task:""}))}
...
return <><div className="user-global-badge-v8782"><span>USUARI ACTIU</span><b>{authUser8779}</b></div><div className={`app-shell ${collapsed?"nav-collapsed":""}`}>{menuOpen&&<div className="overlay" onClick={()=>setMenuOpen(false)}/>}<aside className={`sidebar ${menuOpen?"open":""}`}><div className="sidebar-head"><div className="brand">APP CONTROL D'OBRES</div><div className="active-user-v8780">Usuari: <b>{authUser8779}</b></div><button className="logout-mini-v8778" title="Sortir" onClick={()=>{sessionStorage.removeItem("aco_current_user8779");setClients([]);setObres([]);setOdata({});setAuthUser8779("")}}>Sortir</button><button className="collapse-btn" onClick={()=>setCollapsed(!collapsed)}><Menu size={20}/></button><button className="close-menu" onClick={()=>setMenuOpen(false)}><X/></button></div><nav className="side-nav side-nav-v87163"><MB a={screen==="Inici"} i={<Building2/>} l={tt("Inici","Inicio","Home")} on={()=>nav("Inici")}/><details className="menu-group-v87163" open><summary>Gestió tècnica</summary><MB a={screen==="Clients"||screen==="Fitxa client"} i={<Users/>} l={tt("Clients","Clientes","Clients")} on={()=>nav("Clients")}/><MB a={screen==="Agents"} i={<Users/>} l="Agents" on={()=>nav("Agents")}/><MB a={screen==="Treballs / Expedients"||screen==="Obra"} i={<FolderOpen/>} l={tt("Treballs / Expedients","Trabajos / Expedientes","Jobs / Files")} on={()=>nav("Treballs / Expedients")}/><MB a={screen==="Agenda"} i={<CalendarDays/>} l={tt("Agenda / Calendari","Agenda / Calendario","Calendar")} on={()=>nav("Agenda")}/></details><details className="menu-group-v87163" open><summary>Control econòmic</summary><MB a={screen==="Pressupostos"} i={<ClipboardList/>} l={tt("Pressupostos","Presupuestos","Quotes")} on={()=>nav("Pressupostos")}/><MB a={screen==="Factures"} i={<ReceiptText/>} l={tt("Factures","Facturas","Invoices")} on={()=>nav("Factures")}/><MB a={screen==="Traça"} i={<ReceiptText/>} l={tt("Gestió temps","Gestión tiempo","Time tracking")} on={()=>nav("Traça")}/></details><details className="menu-group-v87163"><summary>Sistema</summary><MB a={screen==="Configuració"} i={<Settings/>} l={tt("Configuració","Configuración","Settings")} on={()=>nav("Configuració")}/><button className="menu-btn" onClick={()=>{sessionStorage.removeItem("aco_current_user8779");setClients([]);setObres([]);setOdata({});setAuthUser8779("")}}><X/><span>Sortir</span></button></details></nav></aside><main className="main"><div className="top-actions-desktop-v87164"><button title="Notificacions / agenda" onClick={()=>nav("Agenda")}><Bell size={18}/><span>Agenda</span></button><button title="Configuració" onClick={()=>nav("Configuració")}><Settings size={18}/><span>Configuració</span></button><button title="Sortir" onClick={()=>{sessionStorage.removeItem("aco_current_user8779");setClients([]);setObres([]);setOdata({});setAuthUser8779("")}}><X size={18}/><span>Sortir</span></button></div><div className="mobile-top mobile-top-v87163"><button onClick={()=>setMenuOpen(true)} className="hamb"><Menu/></button><b>CONTROL D'OBRES</b><div className="mobile-top-actions-v87163"><button title="Notificacions / agenda" onClick={()=>nav("Agenda")}><Bell size={18}/></button><button title="Configuració" onClick={()=>nav("Configuració")}><Settings size={18}/></button></div></div>
...
{screen==="Agents"&&<AgentsDirectori878164 agents={allAgents8749(odata)}/>}
...
function agentRoleText878163(a={}){return String([a.rol,a.funcio,a.funcions,a.categoria,a.tipus,a.empresa,a.nom].filter(Boolean).join(' ')).toLowerCase().normalize('NFD').replace(/[\u0300-\u036f]/g,'')}
...
function agentGroupLabel878164(field=''){
  const f=String(field||'').toLowerCase();
  if(f==='constructor'||f.includes('constructor')||f.includes('contractista'))return 'Constructors / contractistes';
  if(f==='css'||f.includes('seguretat'))return 'Tècnics CSS / seguretat';
  if(f==='do'||f==='deo'||f.includes('direcció')||f.includes('direccio'))return 'Tècnics d’obra';
  if(f.includes('promotor')||f.includes('propiet'))return 'Promotors / propietat';
  return 'Agents disponibles';
}
function agentTypeClass878164(a={}){
  const r=agentRoleText878163(a);
  if(r.includes('constructor')||r.includes('contractista')||r.includes('industrial')||r.includes('constructora'))return 'constructor';
  if(r.includes('promotor')||r.includes('propiet')||r.includes('client'))return 'promotor';
  if(r.includes('coordin')||r.includes('css')||r.includes('seguretat')||r.includes('salut'))return 'css';
  if(r.includes('direccio')||r.includes('direcció')||r.includes('arquitecte')||r.includes('aparellador')||r.includes('tecnic')||r.includes('tècnic')||r.includes('deo')||r.includes('do'))return 'tecnic';
  return 'altres';
}
function roundQuarterHourLabel878164(h=0){return `${roundQuarterHour878163(Number(h)||0).toFixed(2)} h`;}
function tabGroups878164(tabs=[]){
  const groups=[
    {title:'Resum i dades',desc:'Fitxa, agents i tasques',items:['Resum','Dades','Agents','Agenda / Avisos','Tasques','Gestió temps','Rendiment']},
    {title:'Documentació',desc:'Documents, actes i fotos',items:['Documents','Actes','Fotografies','Plànols','Memòria / Informe / Certificat','Renders / Presentació','Tràmits','Seguretat i salut','Tancament / Entrega']},
    {title:'Pressupostos i obra',desc:'Pressupostos, certificacions i Gantt',items:['Honoraris','Pressupostos','Pressupost ràpid','Pressupost obra','Gestió obra','Certificacions obra','Factures','Facturació obra','Amidaments','Industrials / Comparatius']}
  ];
  const used=new Set();
  const out=groups.map(g=>({...g,items:g.items.filter(x=>tabs.includes(x)&&!used.has(x)&&used.add(x))})).filter(g=>g.items.length);
  const rest=tabs.filter(t=>!used.has(t));
  if(rest.length)out.push({title:'Altres',desc:'Resta de pestanyes',items:rest});
  return out;
}
...
<section className="home-pro-v878140">
  <div className="home-pro-title-v878140 home-pro-title-v87165"><span>{todayLabel878141}</span><h1>Hola, {String(currentAppUser8779()||'usuari').replace(/^./,m=>m.toUpperCase())}</h1><p>Resum operatiu: obres actives, tasques pendents, pròximes cites i pressupostos en marxa.</p></div>
  ...
  <div className="home-project-strip-v87165">
    <div className="home-section-label-v87165"><b>Projectes / treballs actius</b><button type="button" onClick={()=>setScreen('Treballs / Expedients')}>Veure tots</button></div>
    <div className="home-project-cards-v87165">
      {obertsMes.slice(0,3).map(({o,count})=><button key={o.id} onClick={()=>openObra(o.id)} className="home-project-card-v87165"><div className="home-project-image-v87165">{o.imatge?<img src={o.imatge}/>:<span>{String(o.nom||'OB').slice(0,2).toUpperCase()}</span>}</div><b>{o.nom}</b><small>{clientName878138(clients,o)}</small><em>{normalizeExpedientStatus878136(o.estat)} · {count||0} mov.</em></button>)}
      <button className="home-project-card-v87165 new" onClick={newObra}><span>+</span><b>Nou expedient</b><small>Crear treball</small></button>
    </div>
  </div>
</section>
...
function PressupostRapid878150(props){
  ...
  function addManual878153(){
    props.setData?.(d=>{
      ...
      return {...d,partides:[...current,{id:"pr-"+Date.now(),codi:String(current.length+1).padStart(2,"0")+".01",ut:"ut",concepte:"Nova partida",desc:"",cap,q:"1,00",pu:"0,00",certAnterior:0,certActual:0,certsByNum:{},tipus:"Pressupost ràpid manual"}],updatedAt:new Date().toISOString()};
    });
  }
  ...
  return <div className="pressupost-rapid-v87150 pressupost-rapid-v87153 pressupost-rapid-v87155 pressupost-rapid-v87160">
    <Card title={`Pressupost ràpid · ${numeroPressupost||referencia||'sense número'}`} action={<div className="actions-inline compact-actions-v87160"><label className="secondary upload-label"><Upload/> Importar Excel<input type="file" accept=".xlsx,.xls" onChange={props.importExcel}/></label><button className="secondary" onClick={()=>{const n=prompt('Núm. pressupost o nom de versió:',numeroPressupost||'');if(n!=null)setMeta878155('pressupostRapidNumero',n)}}>Renombrar</button><button className="secondary" onClick={addManual878153}>+ Partida manual</button><button className="secondary" onClick={saveAsDocument878153}>Guardar a Documents</button><button className="primary" onClick={()=>props.openDoc?.(doc878153())}>Previsualitzar / PDF</button></div>}>
      <div className="rapid-summary-strip-v87160 rapid-summary-strip-v87165"><div><span>Total pressupost</span><b>{money(total)}</b></div><div><span>Partides</span><b>{parts}</b></div><div><span>Client</span><b>{props.client?.nom||props.client?.rao||"Pendent"}</b></div><div><span>Obra</span><b>{props.obra?.nom||"Pendent"}</b></div></div>
      <div className="quick-budget-preview-v87165">
        <div className="quick-budget-preview-head-v87165"><div><b>Vista pressupost</b><span>Com quedarà en PDF. Edita les partides a sota només quan calgui.</span></div><button type="button" className="secondary small" onClick={()=>props.openDoc?.(doc878153())}>Obrir PDF</button></div>
        <div className="quick-budget-table-wrap-v87165"><table><thead><tr><th>Partida</th><th>Ut</th><th>Concepte</th><th>Quantitat</th><th>Preu unit.</th><th>Total</th></tr></thead><tbody>{sortedCapEntries8779(group(rows,'cap')).flatMap(([cap,items])=>[{capRow:true,cap},...sortPartides8779(items)]).map((r,idx)=>r.capRow?<tr key={'cap-'+r.cap} className="quick-cap-row-v87165"><td colSpan="6">{r.cap}</td></tr>:<tr key={r.id||r.codi||idx}><td>{r.codi}</td><td>{r.ut}</td><td><b>{r.concepte}</b>{r.desc&&<small>{r.desc}</small>}</td><td>{qty2(parseNum8770(r.q)||0)}</td><td>{money(parseNum8770(r.pu)||0)}</td><td>{money((parseNum8770(r.q)||0)*(parseNum8770(r.pu)||0))}</td></tr>)}</tbody><tfoot><tr><th colSpan="5">TOTAL</th><th>{money(total)}</th></tr></tfoot></table></div>
      </div>
      <details className="progressive-panel-v87160">
        ...
        <div className="form-grid compact-form-v87160">
          <label><span>Núm. pressupost</span><input value={numeroPressupost} onChange={e=>setMeta878155("pressupostRapidNumero",e.target.value)} placeholder="Ex: 2026-00030"/></label>
    ...
    <details className="progressive-panel-v87160 pressupost-editor-panel-v87160">
      <summary><b>Editar pressupost, capítols i llibreria</b><span>Obre només si has de corregir partides, afegir descomposats o consultar llibreries</span></summary>
      <Pressupost {...props}/>
    </details>
  </div>
}
...
function Obra({obra,client,clients,setClients,data,setData,tab,setTab,setScreen,uploadImage,importExcel,deletePressupostVersion,duplicatePressupostVersion,updateCert,addCertificacio,updateObraFitxa8721,deleteCertificacio8721,updateCertDate8721,updateCertDate,certInfo,setCertInfo,saveCert,openEmail,openDoc,openAgent,openActa,openPartida,openEvent,selectedActaId,setSelectedActaId,timer,setTimer,startTimer,stopTimer,addManualHours,deleteHour,addPressupostTecnic,updatePressupostTecnic,facturarPressupostTecnic,addFacturaTecnica,updateFacturaTecnica,deletePressupostTecnic,deleteFacturaTecnica,deleteObra,allAgents=[],clientHistoricalPartides=[]}){
  ...
  const renderTab=()=> <>
    ...
    {activeTab==="Pressupost ràpid"&&<PressupostRapid878150 data={data} setData={setData} importExcel={importExcel} deletePressupostVersion={deletePressupostVersion} duplicatePressupostVersion={duplicatePressupostVersion} openPartida={openPartida} openEmail={openEmail} openDoc={openDoc} client={client} obra={obra} clientHistoricalPartides={clientHistoricalPartides}/>} 
...
function Pressupost({data,setData,importExcel,deletePressupostVersion,duplicatePressupostVersion,openPartida,openEmail,openDoc,client,clientHistoricalPartides=[],budgetGroups=[],activeBudgetId="principal",selectBudget,addBudget,totalGlobal=0,totalActive=0}){
  ...
  function normalizeLibPartida87115(row={},cap=""){
    return {id:row.id&&String(row.id).startsWith("lib-")?row.id:`lib-${Date.now()}-${Math.random().toString(36).slice(2,7)}`,cap:String(row.cap||cap||"General"),codi:String(row.codi||""),ut:String(row.ut||"ut"),concepte:String(row.concepte||row.resum||row.resumen||"Nova partida"),desc:libDesc87118(row),descompost:String(row.descompost||row.descomposto||row.descomposat||""),pu:+row.pu||0,tipus:String(row.tipus||"Llibreria client")};
  }
  function savePartidaToLibrary87115(row,cap){
    ...
    alert('Partida guardada a la llibreria del client. Obre la llibreria només quan vulguis cercar o reutilitzar partides.');
  }
  ...
  function addLibraryPartidaToBudget87115(item){
    ...
    setCaps(p=>{
      ...
      const row={...item,id:undefined,cap:dest,codi:item.codi|| (base?`${base}.${String(next).padStart(2,"0")}`:""),q:"1,00",pu:qty2(+item.pu||0),descompost:item.descompost||"",tipus:item.tipus||"Llibreria client"};
  ...
  async function importLibraryDescompost878161(id,file){
    ...
    try{
      ...
      setLibraryItems87115(prev=>(prev||[]).map(x=>x.id===id?{...x,descompost:parsed.text,pu:parsed.total||x.pu||0,descompostSource:file.name,descompostSheet:parsed.sheet}:x));
  ...
  const generalLibraryItems87160=useMemo(()=>mergeLibrary87118(readAllLibraries878160()),[libraryItems87115.length,clientLibraryKey87115]);
  const visibleLibraryItems87160=libraryScope87160==="general"?mergeLibrary87118([...(generalLibraryItems87160||[]),...(libraryItems87115||[])]):(libraryItems87115||[]);
  ...
  const libraryFiltered87115=(visibleLibraryItems87160||[]).filter(x=>{
    ...
    const okQ=!q||[x.codi,x.concepte,libDesc87118(x),x.ut,x.tipus,x.origenLlibreria].some(v=>String(v||"").toLowerCase().includes(q));
  ...
  function applyDescompostToPartida878160(cap,i){
    if(!editBudget8760b)return;
    const row=(caps?.[cap]||[])[i]||{};
    const pu=descompostTotal878160(row.descompost||"");
    if(!pu){alert("No he trobat cap import vàlid dins el descomposat. Importa un Excel de descomposat, enganxa les línies o escriu una línia Total.");return;}
    setCaps(p=>{const arr=[...(p[cap]||[])];arr[i]={...arr[i],pu:qty2(pu),puFromDescompost:true};return {...p,[cap]:arr};});
  }
  ...
  return <div className="stack">
    <Card title={`Versions · ${budgetLabel8786(data,data.activeBudgetIdObra||"principal")}`} action={<div className="actions-inline"><button type="button" className="secondary" onClick={startManualBudget87115}>+ Manual des de 0</button><label className="secondary upload-label"><Upload/> Importar Excel<input type="file" onChange={importExcel}/></label></div>}>
...
function MedicioModal8780({row,certNum,initial=[],close,save}){
  ...
  return <Modal title={`Línies de medició · ${row?.codi||""} · CERT. ${certNum}`} close={close}>
    <div className="medicio-modal-v8780">
      <div className="module-note-v8738"><b>{row?.concepte}</b><span>Introdueix línies tipus Presto. Ara multiplica correctament unitats × llargada × amplada × alçada segons la unitat o els camps emplenats.</span></div>
...
function pressupostObraPrintHtml878153(doc,obra,client){
  ...
  return `<!doctype html><html><head><meta charset="utf-8"><title>${escHtmlV8772(doc.title||"Pressupost")}</title><style>@page{size:A4 portrait;margin:14mm}*{box-sizing:border-box}body{font-family:Arial,Helvetica,sans-serif;color:#0f172a;font-size:11px;margin:0;background:white}.page{width:182mm;min-height:269mm;margin:0 auto;background:white}.head{display:grid;grid-template-columns:1.1fr .9fr;gap:12mm;border-bottom:2px solid #0f172a;padding-bottom:9px;margin-bottom:14px}.head h3{margin:0 0 5px;font-size:12px}.head p{margin:0;line-height:1.45;color:#475569}.title{display:grid;grid-template-columns:1fr 55mm;gap:10mm;align-items:start;margin:0 0 14px}.title h1{margin:0 0 7px;font-size:22px;color:#0f2d5c}.title-meta p{margin:0 0 3px;line-height:1.35}.budget-id{border:1px solid #cbd5e1;border-radius:8px;background:#f8fafc;padding:7px 8px;text-align:right}.budget-id small{display:block;color:#64748b;font-weight:900;text-transform:uppercase;font-size:9px;margin-top:3px}.budget-id b{display:block;font-size:14px;color:#0f172a}.budget-id strong{display:block;font-size:17px;color:#0f2d5c;margin-top:6px}table{width:100%;border-collapse:collapse;table-layout:fixed;margin-top:5mm}th,td{border:1px solid #94a3b8;padding:5px 6px;text-align:center;vertical-align:top;font-variant-numeric:tabular-nums;white-space:nowrap}th{background:#dbeafe;color:#0f172a;font-weight:900}.code{text-align:left!important;vertical-align:top!important}.concept{text-align:left!important;vertical-align:top!important;white-space:normal!important;overflow-wrap:anywhere;word-break:normal}.long-desc{margin:3px 0 0;color:#475569;font-size:9px;line-height:1.25;white-space:pre-wrap}.num{text-align:right!important;vertical-align:top!important}.cap td{background:#9fbad4!important;text-align:left!important;font-weight:900!important;border-top:2px solid #0f172a!important;border-bottom:1.5px solid #0f172a!important;white-space:normal!important}.totals{width:70mm;margin-left:auto;margin-top:8mm}.totals div{display:flex;justify-content:space-between;border-top:2px solid #0f172a;padding:8px 0;font-size:16px}.final-notes{display:grid;grid-template-columns:1fr 1fr;gap:8mm;margin-top:9mm}.final-notes div{border:1px solid #cbd5e1;border-radius:8px;padding:8px;background:#f8fafc}.final-notes h3{margin:0 0 5px;font-size:12px;color:#0f2d5c}.final-notes p{margin:0;white-space:pre-wrap;line-height:1.35;color:#334155}.empty{text-align:left!important;color:#64748b}.issuer-v87100{display:grid;grid-template-columns:34mm 1fr;gap:6mm;align-items:start}.issuer-v87100 b,.issuer-v87100 span{display:block;line-height:1.25}.issuer-logo-box-v87100{width:34mm;min-height:18mm;border:1px solid #cbd5e1;display:flex;align-items:center;justify-content:center;background:#fff}.brand-logo-v87100{max-width:32mm;max-height:20mm;object-fit:contain}.brand-logo-placeholder-v87100{font-weight:900;color:#94a3b8;font-size:10px}@media screen{body{background:#e5e7eb;padding:18px}.page{box-shadow:0 2px 18px rgba(15,23,42,.18);padding:14mm;transform-origin:top center}}@media screen and (max-width:900px){body{padding:8px}.page{width:182mm;min-height:269mm;padding:10mm;transform:scale(.54);transform-origin:top left;margin:0 auto}table{font-size:9.5px}th,td{padding:4px 5px}.final-notes{grid-template-columns:1fr}.title{grid-template-columns:1fr 48mm;gap:5mm}}@media print{body{background:white!important;padding:0!important}.page{box-shadow:none!important;padding:0!important;width:182mm!important;min-height:269mm!important;transform:none!important;margin:0!important}}</style></head><body><section class="page"><div class="head">${issuerFiscalBlockHtml87100(client)}<div><h3>Client / promotor</h3><p>${thirdBlock}</p></div></div><div class="title"><div><h1>${escHtmlV8772(doc.title||"PRESSUPOST D’OBRA")}</h1><div class="title-meta">${metaLines}</div></div><div class="budget-id">${numPres?`<small>Núm. pressupost</small><b>${escHtmlV8772(numPres)}</b>`:""}${versioPres?`<small>Versió</small><b>${escHtmlV8772(versioPres)}</b>`:""}<strong>${money(total)}</strong></div></div><table><colgroup><col style="width:18mm"><col style="width:10mm"><col style="width:auto"><col style="width:18mm"><col style="width:22mm"><col style="width:24mm"></colgroup><thead><tr><th>Partida</th><th>Ut</th><th>Concepte</th><th>Quantitat</th><th>Preu unit.</th><th>Total</th></tr></thead><tbody>${body}</tbody></table><div class="totals"><div><span>TOTAL PRESSUPOST</span><b>${money(total)}</b></div></div><div class="final-notes"><div><h3>Observacions</h3><p>${escHtmlV8772(obs)}</p></div><div><h3>Forma de pagament</h3><p>${escHtmlV8772(forma)}</p></div></div></section></body></html>`;
}
