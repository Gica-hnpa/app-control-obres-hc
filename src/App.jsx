...
async function updateObraFitxa8720(patch){setD(obraId,d=>({...d,obra:{...(d.obra||{}),...patch}}))}
...
function Obra({obra,client,data,tab,setTab,setScreen,uploadImage,updateObraFitxa8720,importExcel,deletePressupostVersion,duplicatePressupostVersion,updateCert,addCertificacio,updateCertDate,certInfo,setCertInfo,saveCert,openEmail,openDoc,openAgent,openActa,openPartida,selectedActaId,setSelectedActaId,timer,setTimer,startTimer,stopTimer,addManualHours,deleteHour}){const[estatObra,setEstatObra]=useState(obra.estat||"Pressupostada");const[editObra,setEditObra]=useState(false);let tabs=["Resum","Pressupost obra","Certificacions","Facturació","Actes d’obra","Fotografies","Documents","Honoraris / Temps"];return <div className="obra-page"><div className="obra-topbar"><div><h1>{obra.nom}</h1><p>{client.nom} · {obra.tipologia}</p></div><button className="secondary" onClick={()=>setScreen("Projectes / Obres")}><ArrowLeft/> Tornar</button></div><section className="obra-compact-head">
...
return <Card title="Avisos i notes de l’obra" action={<div className="actions-inline"><button className="primary" onClick={()=>add("Avís")}><Plus/> Nou avís</button><button className="secondary" onClick={()=>add("Nota")}><Plus/> Nova nota</button></div>}>
  ...
  <div className="obra-alerts-summary"><span>Pendents: <b>{pending.length}</b></span><span>Fets: <b>{done.length}</b></span></div>{editObra8720&&<EditObraModal8720 obra={data.obra||obra} close={()=>setEditObra8720(false)} save={(patch)=>{updateObraFitxa8720?.(patch);setEditObra8720(false)}}/>}
</Card>
}
...
function EditObraModal8720({obra,close,save}){
...
const[dateDraft8720,setDateDraft8720]=useState({});
...
function dateVal8720(c){return dateDraft8720[c.id]??fmtDate8714(c.data)}
function saveDates8720(){certs.forEach(c=>{if(dateDraft8720[c.id]!==undefined)updateCertDate?.(c.id,dateDraft8720[c.id])});setDateDraft8720({})}
...
return <div className="stack">
<Card title="Certificacions realitzades" action={<div className="actions-inline"><button className="secondary" onClick={saveDates8720}>Guardar dates</button><button className="primary" onClick={()=>{addCertificacio?.();setCertMode8711("emplenar")}}>+ Nova certificació</button></div>}>
  <div className="version-list">{certs.length===0?<Empty text="Aquesta obra encara no té certificacions guardades."/>:certs.map(c=><button className={`version-row ${selected===c.id?"active":""}`} key={c.id} onClick={()=>{setSelected(c.id);setCertMode8711("resum")}}><b>Certificació {c.numero}</b><input className="cert-date-input-v8720" value={dateVal8720(c)} onClick={e=>e.stopPropagation()} onFocus={e=>e.stopPropagation()} onChange={e=>setDateDraft8720(d=>({...d,[c.id]:e.target.value}))}/><strong>{money(rows.reduce((s,r)=>s+qFor(r,+c.numero)*(+r.pu||0),0))}</strong><em>{selected===c.id?"Seleccionada":"Veure"}</em></button>)}</div>
</Card>
