...
{screen==="Treballs / Expedients"&&<Projectes byClient={byClient} clients={clients} openObra={openObra} f={{os,setOs,oc,setOc,oy,setOy,ost,setOst}} newObra={()=>setModal("obra")}/>}
...
const serveisBase=["Project Manager","Direcció d’obra","Pressupost","Certificació","Visita d’obra","Informe tècnic","Reforma habitatge unifamiliar","Reforma interior","Rehabilitació façana","Obra nova","Piscina","Ascensor","Manteniment","Altres"];
...
return <Modal title="Modificar fitxa de l’obra" close={close}>
  <div className="form-grid fitxa-form-v8727">
    ...
    <label><span>Tipus servei</span><select value={f.tipologia||""} onChange={e=>ch("tipologia",e.target.value)}>
...
function Obra({obra,client,clients,data,tab,setTab,setScreen,uploadImage,importExcel,deletePressupostVersion,duplicatePressupostVersion,updateCert,addCertificacio,updateObraFitxa8721,deleteCertificacio8721,updateCertDate8721,updateCertDate,certInfo,setCertInfo,saveCert,openEmail,openDoc,openAgent,openActa,openPartida,selectedActaId,setSelectedActaId,timer,setTimer,startTimer,stopTimer,addManualHours,deleteHour}){const[estatObra,setEstatObra]=useState(obra.estat||"Pressupostada");const[editObra,setEditObra]=useState(false);let tabs=["Resum","Pressupost tècnic-client","Certificacions obra","Facturació tècnic-client","Actes","Fotografies","Documents","Honoraris / Temps"];return <div className="obra-page">{editObra&&<EditObraModal8725 obra={obra} clients={clients||[]} close={()=>setEditObra(false)} save={(patch)=>{updateObraFitxa8721?.(patch);setEditObra(false)}}/>}<div className="obra-topbar"><div><h1>{obra.nom}</h1><p>{client.nom} · {obra.tipologia}</p></div><button className="secondary" onClick={()=>setScreen("Treballs / Expedients")}><ArrowLeft/> Tornar</button></div><section className="obra-compact-head">
