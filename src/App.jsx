    ...
    <div className="actes-layout-v8761">
      ...
      <div className="acta-editor-v8761">
        ...
        {draft&&<>
          ...
          {actaPanel8799==="Agents"&&<div className="agents-box-v8764 agents-box-v87102">
            <div className="section-head-v8764"><div><b>Biblioteca d’agents</b><span>Cerca i afegeix només els agents que vols incorporar a aquesta acta.</span></div><button type="button" className="secondary" onClick={()=>setShowNewAgent87102(v=>!v)}>{showNewAgent87102?"Tancar alta":"+ Crear agent nou"}</button></div>
            <div className="agent-search-box-v87102"><input value={agentSearch87102} onChange={e=>setAgentSearch87102(e.target.value)} placeholder="Cerca agent per nom, empresa, rol o email..."/><select value="" onChange={e=>{addAgentToDraft8799(e.target.value);e.target.value=""}}><option value="">+ Afegir agent de la biblioteca</option>{agents.filter(a=>`${a.nom||""} ${a.rol||""} ${a.empresa||""} ${a.email||""}`.toLowerCase().includes(agentSearch87102.toLowerCase())).map(a=><option key={a.id} value={a.id}>{(draft.agentIds||[]).includes(a.id)?"✓ ":""}{a.nom} · {a.rol||""} · {a.empresa||""}</option>)}</select></div>
            {showNewAgent87102&&<div className="new-agent-box-v87102"><h4>Crear agent nou</h4><div className="agent-form-v8764"><input list="agent-names-v87102" placeholder="Nom agent" value={agentForm8764.nom} onChange={e=>{const val=e.target.value;const found=agents.find(a=>String(a.nom||"").toLowerCase()===val.toLowerCase());setAgentForm8764(p=>found?{...p,nom:val,rol:found.rol||p.rol,empresa:found.empresa||p.empresa,email:found.email||p.email,telefon:found.telefon||p.telefon}:{...p,nom:val})}}/><datalist id="agent-names-v87102">{agents.map(a=><option key={a.id} value={a.nom}/>)}</datalist><input list="agent-rols-v87102" placeholder="Rol" value={agentForm8764.rol} onChange={e=>setAgentForm8764(p=>({...p,rol:e.target.value}))}/><datalist id="agent-rols-v87102">{[...new Set(agents.map(a=>a.rol).filter(Boolean))].map(x=><option key={x} value={x}/>)}</datalist><input list="agent-companies-v87102" placeholder="Empresa" value={agentForm8764.empresa} onChange={e=>setAgentForm8764(p=>({...p,empresa:e.target.value}))}/><datalist id="agent-companies-v87102">{[...new Set(agents.map(a=>a.empresa).filter(Boolean))].map(x=><option key={x} value={x}/>)}</datalist><input placeholder="Email" value={agentForm8764.email} onChange={e=>setAgentForm8764(p=>({...p,email:e.target.value}))}/><input placeholder="Telèfon" value={agentForm8764.telefon||""} onChange={e=>setAgentForm8764(p=>({...p,telefon:e.target.value}))}/><button type="button" className="primary" onClick={addAgent8764}>Guardar agent</button></div></div>}
            ...
            {(()=>{const ids=[...new Set(actes.filter(a=>a.id!==draft.id).flatMap(a=>(normalizeActa8768(a,agents).agentIds||[])))].filter(id=>!(draft.agentIds||[]).includes(id));return ids.length>0&&<div className="prev-agents-v87102"><h4>Repetir agents d’actes anteriors</h4><div className="check-grid">{ids.map(id=>{const a=agents.find(x=>x.id===id);return a?<label className="check-row" key={id}><input type="checkbox" onChange={()=>toggleAgent(id)}/><span>{a.nom} · {a.rol||""} · {a.empresa||""}</span></label>:null})}</div></div>})()}
            <div className="agent-library-edit-v87100"><h4>Llistat complet de biblioteca</h4><p>Edita o elimina agents de la biblioteca. Només s'incorporen a l'acta si els selecciones.</p>{agents.filter(a=>`${a.nom||""} ${a.rol||""} ${a.empresa||""} ${a.email||""}`.toLowerCase().includes(agentSearch87102.toLowerCase())).map(ag=><div className="agent-library-row-v87100" key={ag.id}><input value={ag.nom||""} onChange={e=>updateAgent8764(ag.id,"nom",e.target.value)} placeholder="Nom"/><input value={ag.rol||""} onChange={e=>updateAgent8764(ag.id,"rol",e.target.value)} placeholder="Rol"/><input value={ag.empresa||""} onChange={e=>updateAgent8764(ag.id,"empresa",e.target.value)} placeholder="Empresa"/><input value={ag.email||""} onChange={e=>updateAgent8764(ag.id,"email",e.target.value)} placeholder="Email"/><button type="button" className="danger small" onClick={()=>deleteAgent8764(ag.id)}>Eliminar</button></div>)}</div>
          </div>}
