...
async function updateObraFitxa8721b(patch){setD(obraId,d=>({...d,obra:{...(d.obra||{}),...patch}}))}
...
function deleteCertificacio8721a(id){
  if(!confirm("Eliminar aquesta certificació?")) return;
  setD(obraId,d=>({...d,certificacions:(d.certificacions||[]).filter(c=>c.id!==id)}))
}
...
function Obra({obra,client,data,tab,setTab,setScreen,uploadImage,updateObraFitxa8721b,importExcel,deletePressupostVersion,duplicatePressupostVersion,updateCert,addCertificacio,deleteCertificacio8721a,updateObraFitxa8721,deleteCertificacio8721,updateCertDate8721,updateCertDate,certInfo,setCertInfo,saveCert,openEmail,openDoc,openAgent,openActa,openPartida,selectedActaId,setSelectedActaId,timer,setTimer,startTimer,stopTimer,addManualHours,deleteHour}){const[estatObra,setEstatObra]=useState(obra.estat||"Pressupostada");const[editObra,setEditObra]=useState(false);let tabs=["Resum","Pressupost obra","Certificacions","Facturació","Actes d’obra","Fotografies","Documents","Honoraris / Temps"];return <div className="obra-page"><div className="obra-topbar"><div><h1>{obra.nom}</h1><p>{client.nom} · {obra.tipologia}</p></div><button className="secondary" onClick={()=>setScreen("Projectes / Obres")}><ArrowLeft/> Tornar</button></div><section className="obra-compact-head">
...
function EditObraModal8721b({obra,close,save}){
const[f,setF]=useState(()=>({...obra}));
function ch(k,v){setF(x=>({...x,[k]:v}))}
return <Modal title="Modificar fitxa de l’obra" close={close}>
  <div className="form-grid">
...
function Resum({data,updateObraFitxa8721b}){
const[editObra8721b,setEditObra8721b]=useState(false);
...
return <div className="stack">
  <div className="fitxa-edit-toolbar-v8721b"><button className="secondary" onClick={()=>setEditObra8721b(true)}>Modificar fitxa obra</button></div>
  {editObra8721b&&<EditObraModal8721b obra={data.obra||{}} close={()=>setEditObra8721b(false)} save={(patch)=>{updateObraFitxa8721b?.(patch);setEditObra8721b(false)}}/>}
...
function Cert({data,updateCert,addCertificacio,deleteCertificacio8721a,deleteCertificacio8721,updateCertDate8721,updateCertDate,ci,setCi,saveCert,openEmail,openDoc}){
