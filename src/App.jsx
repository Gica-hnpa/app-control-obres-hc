...
function add(tipus="Avís"){let n={id:"oa"+Date.now(),tipus,prioritat:"Normal",titol:tipus==="Avís"?"Nou avís":"Nova nota / visita",text:"",limit:"",fet:false};setItems(p=>[n,...p]);setEditing(n.id)}
...
return <Card title="Avisos i notes de l’obra" action={<div className="actions-inline"><button className="primary" onClick={()=>add("Avís")}><Plus/> Nou avís</button><button className="secondary" onClick={()=>add("Nota")}><Plus/> Nova nota / visita</button></div>}>
      ...
      <div className="calendar-grid">
        ...
        {Array.from({length:total}).map((_,i)=>{
          ...
          return <button key={day} onClick={()=>setSelDay(day)} className={`day ${selDay===day?"active":""}`}>
            <b>{String(day).padStart(2,"0")}/{String(calM+1).padStart(2,"0")}/{String(calY).slice(-2)}</b>
...
<textarea className="avisos-obs" value={x.observacions} onChange={e=>upd(x.id,"observacions",e.target.value)} placeholder="Observacions / adreça visita completes"/>
...
<select value={x.estat} onChange={e=>upd(x.id,"estat",e.target.value)}><option>Pendent</option><option>Fet</option><option>Rebutjat</option></select>
<textarea value={x.observacions} onChange={e=>upd(x.id,"observacions",e.target.value)} placeholder="Observacions / adreça visita completes"/>
<div className="row-actions"><button className="secondary" onClick={()=>setEditing(null)}>Guardar</button><button className="danger" onClick={()=>del(x.id)}>Eliminar</button></div>
</>:<>
