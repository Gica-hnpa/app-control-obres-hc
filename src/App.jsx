    ...
    <div className="actes-layout-v8761">
      ...
      <div className="acta-editor-v8761">
        ...
        {draft&&<>
          ...
          {actaPanel8799==="Agents"&&<div className="agents-box-v8764">
            ...
            <div className="acta-agent-selector-v8799">
              <select value="" onChange={e=>{addAgentToDraft8799(e.target.value);e.target.value=""}}><option value="">+ Selecciona agent de la biblioteca</option>{agents.filter(a=>!(draft.agentIds||[]).includes(a.id)).map(a=><option key={a.id} value={a.id}>{a.nom} · {a.rol||""} · {a.empresa||""}</option>)}</select>
              <div className="selected-agent-chips-v8799">{selectedAgents.length===0?<span className="muted">Cap agent seleccionat per aquesta acta.</span>:selectedAgents.map(a=><span key={a.id}>{a.nom}<button type="button" onClick={()=>toggleAgent(a.id)}>×</button></span>)}</div>
            </div>
            <div className="agent-library-edit-v87100"><h4>Biblioteca d’agents de l’expedient</h4><p>Edita o elimina agents sense haver-los d’afegir necessàriament a aquesta acta.</p>{agents.map(ag=><div className="agent-library-row-v87100" key={ag.id}><input value={ag.nom||""} onChange={e=>updateAgent8764(ag.id,"nom",e.target.value)} placeholder="Nom"/><input value={ag.rol||""} onChange={e=>updateAgent8764(ag.id,"rol",e.target.value)} placeholder="Rol"/><input value={ag.empresa||""} onChange={e=>updateAgent8764(ag.id,"empresa",e.target.value)} placeholder="Empresa"/><input value={ag.email||""} onChange={e=>updateAgent8764(ag.id,"email",e.target.value)} placeholder="Email"/><button type="button" className="danger small" onClick={()=>deleteAgent8764(ag.id)}>Eliminar</button></div>)}</div>
          </div>}
...
function addClient(e){e.preventDefault();let f=new FormData(e.currentTarget),id="client-"+Date.now();let nom=String(f.get("nom")||"").trim(),rao=String(f.get("rao")||"").trim(),nif=String(f.get("nif")||"").trim(),adreca=String(f.get("adreca")||"").trim(),cp=String(f.get("codiPostal")||"").trim(),pob=String(f.get("poblacio")||"").trim();if(!nom||!rao||!nif||!adreca||!cp||!pob){alert("Cal emplenar els camps obligatoris: nom, raó social/nom fiscal, NIF/CIF, adreça, codi postal i població.");return}let c={id,nom,rao,tipus:f.get("tipus"),contacte:f.get("contacte")||"",nif,email:f.get("email")||"",telefon:f.get("telefon")||"",adreca,codiPostal:cp,poblacio:pob,provincia:f.get("provincia")||provinciaForCp8773(cp)||provinciaForPoblacio8773(pob)||"",color:"blue",logo:f.get("logoPreview")||""};learnCpPoblacio8775(c.codiPostal,c.poblacio);setClients(p=>[c,...p]);setModal(null);openClient(id)}
function addObra(e){
  e.preventDefault();
...
function FitxaClient({client,obres,openObra,back}){
  ...
  function save(){
    if(!form.nom||!form.rao||!form.nif||!form.adreca||!form.codiPostal||!form.poblacio){alert("Cal emplenar els camps obligatoris: nom, raó social/nom fiscal, NIF/CIF, adreça, codi postal i població.");return}
    learnCpPoblacio8775(form.codiPostal,form.poblacio);
  ...
  return <div className="stack client-fitxa-v8773"><button className="secondary" onClick={back}>← Tornar</button><Card title={form.nom||client.nom} action={<div className="actions-inline"><button className="secondary" onClick={()=>setEdit(!edit)}>{edit?"Tancar edició":"Editar client"}</button>{edit&&<button className="primary" onClick={save}>Guardar client</button>}</div>}>
    ...
    <div className="form-grid">
      ...
      <label><span>NIF/CIF fiscal (empresa/autònom)</span><input value={form.nif||""} onChange={e=>set("nif",e.target.value)} disabled={!edit}/></label>
...
function issuerFiscalName87100(client){return client?.rao||client?.nomFiscal||client?.nom||"Nom fiscal pendent"}
...
function issuerFiscalBlockHtml87100(client){return `<div class="issuer-v87100"><div class="issuer-logo-box-v87100">${issuerLogoHtml87100(client)}</div><div><b>${escHtmlV8772(issuerFiscalName87100(client))}</b><span>NIF/CIF: ${escHtmlV8772(client?.nif||"Pendent")}</span><span>${escHtmlV8772(client?.adreca||"")}</span><span>${escHtmlV8772([client?.codiPostal,client?.poblacio,client?.provincia?`(${client.provincia})`:""].filter(Boolean).join(" "))}</span><span>${escHtmlV8772(client?.email||"")}${client?.telefon?` · ${escHtmlV8772(client.telefon)}`:""}</span></div></div>`}
...
function FormClient({onSubmit}){
  ...
  return <form onSubmit={onSubmit} className="client-form-v8799"><DatalistCP8773/>
    ...
    <div className="form-grid client-form-grid-v8799">
      ...
      <label><span>Nom comercial / nom visible *</span><input name="nom" required placeholder="Ex. SOCOTERM, BRAVA, Joan Puig..."/></label>
      <label><span>Raó social / nom fiscal *</span><input name="rao" required placeholder="Empresa, autònom o persona física"/></label>
      <label><span>NIF/CIF fiscal (empresa/autònom) *</span><input name="nif" required placeholder="NIF/CIF fiscal"/></label>
...
function DocViewer({doc,obra,client,close,email}){
  ...
  useEffect(()=>{if(doc?.autoPrint){const t=setTimeout(()=>printIsolated(),120);return()=>clearTimeout(t)}},[doc?.autoPrint]);
  return <Modal title={doc.autoPrint?`Impressió · ${doc.title}`:doc.title} close={close}>
