...
const[docs,setDocs]=useState(()=>JSON.parse(localStorage.getItem("aco_docs_meta_v89")||"null")||JSON.parse(localStorage.getItem("aco_docs_meta_v38")||"null")||JSON.parse(localStorage.getItem("aco_docs_meta_v33")||"null")||[{id:"d1",nom:"Pressupost oficial PDF",tipus:"PDF",data:"10/05/2026",hasFile:false,size:0},{id:"d2",nom:"Pla de seguretat.pdf",tipus:"PDF",data:new Date().toLocaleDateString("ca-ES"),hasFile:false,size:0}]);
const[cfg,setCfg]=useState(()=>getStorageCfg());
const[showCfg,setShowCfg]=useState(false);
const[status,setStatus]=useState("");
const[budget,setBudget]=useState(null);
useEffect(()=>{localStorage.setItem("aco_docs_meta_v89",JSON.stringify(docs))},[docs]);
...
function isExcelName(name){return /\.(xlsx|xls|xlsm|csv)$/i.test(name||"")}
function num(v){if(typeof v==="number")return v;let s=String(v??"").replace(/\./g,"").replace(",",".").replace(/[^\d.-]/g,"");let n=parseFloat(s);return Number.isFinite(n)?n:0}
function clean(v){return String(v??"").trim()}
function rowHasData(r){return r.some(c=>clean(c)!=="")}
async function parseExcelBudget(file){
  const ab=await file.arrayBuffer();
  const wb=XLSX.read(ab,{type:"array"});
  const sheet=wb.Sheets[wb.SheetNames[0]];
  const data=XLSX.utils.sheet_to_json(sheet,{header:1,defval:""});
  let headerIndex=data.findIndex(r=>r.map(x=>String(x).toLowerCase()).some(x=>x.includes("partida")||x.includes("concepte")||x.includes("descrip")||x.includes("quant")));
  if(headerIndex<0)headerIndex=0;
  const header=(data[headerIndex]||[]).map(x=>String(x).toLowerCase());
  function find(keys, fallback){let i=header.findIndex(h=>keys.some(k=>h.includes(k)));return i>=0?i:fallback}
  const idx={
    partida:find(["partida","codi","código"],0),
    ut:find(["ut","unitat","unidad"],1),
    concepte:find(["concepte","descrip","resum"],2),
    quant:find(["quant","cantidad","amid","canpres"],3),
    preu:find(["preu","precio","prpres","preu/unitat"],4),
    total:find(["total","import","imppres"],5)
  };
  const rows=data.slice(headerIndex+1).filter(rowHasData).map((r,i)=>{
    const q=num(r[idx.quant]), pu=num(r[idx.preu]);
    let total=num(r[idx.total]);
    if(!total&&q&&pu)total=q*pu;
    return {id:"b"+i,partida:clean(r[idx.partida])||String(i+1).padStart(2,"0"),ut:clean(r[idx.ut])||"",concepte:clean(r[idx.concepte])||clean(r.find(x=>String(x).length>2))||"Partida",quant:q,preu:pu,total}
  }).filter(r=>r.concepte && (r.total || r.quant || r.preu || r.partida));
  const total=rows.reduce((s,r)=>s+(+r.total||0),0);
  return {sheet:wb.SheetNames[0],rows,total,created:new Date().toLocaleString("ca-ES")};
}
async function add(e){
  ...
  let excelBudget=null;
  try{
    if(isExcelName(f.name)){
      setStatus("Llegint Excel de pressupost...");
      excelBudget=await parseExcelBudget(f);
      setBudget({docId:id,nom:f.name,...excelBudget});
    }
  }catch(err){
    excelBudget={error:"No s’ha pogut llegir automàticament l’Excel: "+String(err?.message||err),rows:[],total:0};
    setStatus(excelBudget.error);
  }
  try{
    if(isStorageReady(cfg)){
      setStatus("Pujant document a Supabase Storage...");
      let up=await uploadToSupabaseStorage(f,{id,obraId:"obra-actual"});
      setDocs(p=>[{...baseMeta,storage:"supabase",path:up.path,url:up.publicUrl,hasFile:true,budget:excelBudget},...p]);
      setStatus(excelBudget?"Excel llegit i document pujat.":"Document pujat a Supabase Storage.");
      return;
    }
    await saveDocFile(id,f);
    setDocs(p=>[{...baseMeta,storage:"indexeddb",hasFile:true,budget:excelBudget},...p]);
    setStatus(excelBudget?"Excel llegit i guardat en local.":"Document guardat en local IndexedDB.");
  }catch(err){
    try{await saveDocFile(id,f);setDocs(p=>[{...baseMeta,storage:"indexeddb",hasFile:true,budget:excelBudget,error:String(err?.message||err)},...p]);setStatus("No s’ha pogut pujar a Supabase. S’ha guardat localment en aquest navegador.");}
    catch(e2){setDocs(p=>[{...baseMeta,storage:"registre",hasFile:false,budget:excelBudget,error:String(err?.message||err)},...p]);setStatus("No s’ha pogut guardar l’original. Revisa Storage o mida del fitxer.")}
  }
}
...
async function previewBudget(d){
  if(d.budget&&d.budget.rows&&d.budget.rows.length){setBudget({docId:d.id,nom:d.nom,...d.budget});return}
  if(d.hasFile){
    let file=await getDocFile(d.id);
    if(file&&isExcelName(d.nom)){
      try{
        let b=await parseExcelBudget(file);
        setDocs(p=>p.map(x=>x.id===d.id?{...x,budget:b}:x));
        setBudget({docId:d.id,nom:d.nom,...b});
        setStatus("Excel llegit correctament.");
        return;
      }catch(err){setStatus("No s’ha pogut llegir l’Excel: "+String(err?.message||err));}
    }
  }
  setStatus("Aquest document no té pressupost Excel llegible.");
}
async function remove(d){
  ...
  if(budget?.docId===d.id)setBudget(null);
  setStatus("Document eliminat.");
}
...
return <div className="docs-budget-v89">
<Card title="Documents adjunts d’obra" action={<div className="actions-inline"><button className="secondary" onClick={()=>setShowCfg(!showCfg)}>Configurar Storage</button><label className="primary upload-label"><Plus/> Nou document<input type="file" accept=".pdf,.doc,.docx,.xls,.xlsx,.xlsm,.csv,image/*" onChange={add}/></label></div>}>
...
<div className={`doc-mode-note ${isStorageReady(cfg)?"ok":""}`}><b>Estat documents:</b> {isStorageReady(cfg)?"Supabase Storage configurat.":"Storage no configurat: originals en local IndexedDB."} {status&&<span> · {status}</span>}</div>
<div className="list">{docs.map(d=><div className="doc-row doc-row-v89" key={d.id}><div><strong>{d.nom}</strong><span>{d.tipus} · {d.data} · {sizeTxt(d.size)} · {storageLabel(d)}</span>{d.budget?.rows?.length>0&&<em>Pressupost llegit: {d.budget.rows.length} partides · {money(d.budget.total||0)}</em>}</div><div className="doc-actions-v89"><button onClick={()=>openOriginal(d)}>Obrir</button>{isExcelName(d.nom)&&<button className="primary small" onClick={()=>previewBudget(d)}>Veure pressupost</button>}<button onClick={()=>openEmail(d.nom)}>Enviar</button><button className="danger small" onClick={()=>remove(d)}>Eliminar</button></div></div>)}</div>
</Card>
{budget&&<Card title={`Previsualització pressupost · ${budget.nom}`} action={<button className="secondary" onClick={()=>setBudget(null)}>Tancar</button>}>
  {budget.error?<div className="alert">{budget.error}</div>:<div className="budget-preview-v89">
    <div className="budget-summary-v89"><b>{budget.rows.length} partides detectades</b><strong>{money(budget.total||0)}</strong><span>Full: {budget.sheet}</span></div>
    <table><thead><tr><th>Partida</th><th>Ut</th><th>Concepte / Descripció</th><th>Quantitat</th><th>Preu/Unitat</th><th>Total</th></tr></thead><tbody>{budget.rows.map(r=><tr key={r.id}><td>{r.partida}</td><td>{r.ut}</td><td>{r.concepte}</td><td>{qty2(r.quant)}</td><td>{money(r.preu)}</td><td>{money(r.total)}</td></tr>)}</tbody><tfoot><tr><th colSpan="5">TOTAL PRESSUPOST</th><th>{money(budget.total||0)}</th></tr></tfoot></table>
  </div>}
</Card>}
</div>
}
