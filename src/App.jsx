...
function FitxaDadesTab8769({obra,client,save}){
  const [form,setForm]=useState(()=>({...obra}));
  useEffect(()=>setForm({...obra}),[obra.id]);
  function upd(k,v){setForm(p=>({...p,[k]:v}))}
  return <Card title="Dades generals de l’expedient" action={<button className="primary" onClick={()=>save?.(form)}>Guardar dades</button>}>
    <div className="form-grid">
      ...
      <label><span>Tipus de feina</span><select value={form.tipusTreball||form.tipologia||"Altres"} onChange={e=>upd("tipusTreball",e.target.value)}>{WORK_TYPES8737.map(t=><option key={t}>{t}</option>)}</select></label>
      <label><span>Promotor / propietat</span><input value={form.propietat||""} onChange={e=>upd("propietat",e.target.value)}/></label>
      <label><span>Constructor</span><input value={form.constructor||""} onChange={e=>upd("constructor",e.target.value)}/></label>
      <label><span>Direcció d’obra</span><input value={form.do||""} onChange={e=>upd("do",e.target.value)}/></label>
      <label><span>Direcció execució</span><input value={form.deo||""} onChange={e=>upd("deo",e.target.value)}/></label>
      <label><span>Coordinació S+S</span><input value={form.css||""} onChange={e=>upd("css",e.target.value)}/></label>
      <label><span>Adreça</span><input value={form.adreca||""} onChange={e=>upd("adreca",e.target.value)}/></label>
      <label><span>Població</span><input value={form.poblacio||""} onChange={e=>upd("poblacio",e.target.value)}/></label>
