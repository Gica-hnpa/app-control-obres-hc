            ...
            {open[cap]&&<div className="budget-v25-lines">
              ...
              {sortPartides8779(items).map((r)=>{
                ...
                return <div className={`budget-v25-line ${editBudget8760b&&!rowOpenV87173?"budget-row-collapsed-v87173":"budget-row-open-v87173"}`} key={i}>
                  ...
                  {rowOpenV87173?<input type="text" inputMode="decimal" value={editBudget8760b?(r.q??""):qty2(parseNum8770(r.q)||0)} onFocus={e=>e.currentTarget.select()} onChange={e=>upd(cap,i,"q",e.target.value)} onBlur={e=>upd(cap,i,"q",qty2(parseNum8770(e.target.value)||0))}/>:<span className="budget-static-cell-v87173 num">{qty2(parseNum8770(r.q)||0)}</span>}
                  ...
                  <b>{money(t)}</b>
                  {editBudget8760b?<div className="budget-line-actions-v878128 budget-line-actions-v87173"><button type="button" className="secondary small" onClick={()=>toggleBudgetRow87173(cap,i)}>{rowOpenV87173?"Tancar partida":"Editar partida"}</button><button type="button" className="secondary small" onClick={()=>openDescompostModal87173(cap,i)}>Descomposat</button>{rowOpenV87173&&<><select value={cap} title="Canviar de capítol" onChange={e=>movePartidaCap878128(cap,i,e.target.value)}>{sortedCapEntries8779(caps).map(([c])=><option key={c} value={c}>{c}</option>)}</select><button type="button" className="secondary small" onClick={()=>{setLibraryOpen87115(true);setLibrarySearch87115(String(r.concepte||""));setLibraryTargetCap87115(cap)}}>Llibreria</button><button type="button" className="danger small budget-delete-line-v878125" onClick={()=>deletePartida878125(cap,i)}>Eliminar</button></>}</div>:<span/>}
                </div>
              })}
