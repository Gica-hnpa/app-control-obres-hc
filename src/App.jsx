...
function PressupostRapid878150(props){
  ...
  function doc878153(){return {type:"pressupostobra",title:"PRESSUPOST D’OBRA",numeroPressupost,referencia,dataPressupost,versioPressupost,tercerNom,tercerNif,tercerAdreca,tercerEmail,subtitle:`${parts} partides · ${money(total)}`,rows,total,data:new Date().toLocaleDateString("ca-ES"),observacions,formaPagament}}
    ...
    <Card title="Pressupost ràpid" action={<div className="actions-inline"><label className="secondary upload-label"><Upload/> Importar Excel<input type="file" accept=".xlsx,.xls" onChange={props.importExcel}/></label><button className="secondary" onClick={addManual878153}>+ Partida manual</button><button className="secondary" onClick={saveAsDocument878153}>Guardar a Documents</button><button className="primary" onClick={()=>props.openDoc?.(doc878153())}>Previsualitzar / imprimir PDF</button></div>}>
      ...
      <details className="budget-observacions-v87155" open>
        ...
        <div className="form-grid">
          ...
          <label><span>Referència</span><input value={referencia} onChange={e=>setMeta878155("pressupostRapidReferencia",e.target.value)} placeholder="Ex: Pressupost de SATE"/></label>
...
function pressupostObraPrintHtml878153(doc,obra,client){
  ...
  const metaLine=[ref?`<b>Referència:</b> ${escHtmlV8772(ref)}`:"",dataPres?`<b>Data:</b> ${escHtmlV8772(fmtAppDate8748(dataPres)||dataPres)}`:"",versioPres?`<b>Versió:</b> ${escHtmlV8772(versioPres)}`:""].filter(Boolean).join(" · ");
  let lastCap="__none__";
