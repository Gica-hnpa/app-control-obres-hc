...
function SafeFormExpedient8751({clients,onSubmit,allAgents=[]}){
  ...
  function NewExpAgentPicker878159({field,label}){
    ...
    const filteredAgentChoices878163=agentOptionsForField878163(agentChoices878159,field);
    const known=filteredAgentChoices878163.some(a=>String(a.nom||'')===String(current));
    const selected=known?current:(current&&current!=='Pendent'?'__custom__':'Pendent');
    return <label><span>{label}</span><select value={selected} onChange={e=>{const v=e.target.value;if(v==='__custom__')setAgentVal878159(field,'');else setAgentVal878159(field,v||'Pendent')}}><option value='Pendent'>Pendent / no cal ara</option>{filteredAgentChoices878163.map(a=><option key={field+a.id} value={a.nom}>{a.nom} · {a.empresa||a.rol||'Agent'}</option>)}<option value='__custom__'>+ Crear / escriure nou</option></select>{selected==='__custom__'&&<input className='mt-6-v8773' value={current==='Pendent'?'':current} onChange={e=>setAgentVal878159(field,e.target.value)} placeholder='Nom del tècnic o empresa'/>}<input type='hidden' name={field} value={current}/></label>
  }
  function setTipus(v){
...
function FitxaDadesTab8769({obra,client,clients=[],setClients,data={},save,allAgents=[],setData,openAgent}){
  ...
  function AgentPicker({field,label,roleHint}){
    ...
    const filteredAgentChoices878163=agentOptionsForField878163(agentChoices,field);
    const known=filteredAgentChoices878163.find(a=>String(a.nom||"")===String(current));
    const selected=known?known.id:(current&&current!=="Pendent"?"__custom__":"Pendent");
    return <label><span>{label}</span><select value={selected} onChange={e=>{const v=e.target.value;if(v==="Pendent"){upd(field,"Pendent");return}if(v==="__custom__"){upd(field,"");return}const ag=filteredAgentChoices878163.find(a=>a.id===v);if(ag){upd(field,ag.nom);addAgentToObraIfNeeded878159(ag)}}}><option value="Pendent">Pendent / no assignat</option>{filteredAgentChoices878163.map(a=><option key={field+a.id} value={a.id}>{a.nom} · {a.empresa||a.rol||"Agent"}</option>)}<option value="__custom__">+ Crear / escriure nou</option></select>{(selected==="__custom__"||(!known&&current&&current!=="Pendent"))&&<div className="agent-inline-create-v87159"><input value={current==="Pendent"?"":current} onChange={e=>upd(field,e.target.value)} placeholder="Nom del tècnic o empresa"/><button type="button" className="secondary small" onClick={()=>{const nom=String(form[field]||"").trim();if(!nom){alert("Escriu el nom de l'agent.");return}const ag={id:"agent-"+Date.now(),nom,rol:roleHint||label,empresa:nom,email:"",telefon:"",nif:"",adreca:"",collegiat:"",updatedAt:new Date().toISOString()};addAgentToObraIfNeeded878159(ag)}}>Crear fitxa agent</button></div>}</label>
  }
