...
<Card title="Factures proforma de certificacions">
  <div className="fact-grid-v83">
    <div className="fg-head">Proforma</div><div className="fg-head">Data</div><div className="fg-head">Base</div><div className="fg-head">IVA</div><div className="fg-head">Ret.</div><div className="fg-head">Ded.</div><div className="fg-head">Total</div><div className="fg-head">Accions</div>
    {proformes.length===0&&<div className="fg-empty"><Empty text="Encara no hi ha proformes."/></div>}
    {proformes.map(f=>{let c=calc(f);return <React.Fragment key={f.pfId}>
      <div className={current?.pfId===f.pfId?"sel":""}><b>{f.numero}</b><small>Cert. {f.numeroCert}</small></div>
      <div className={current?.pfId===f.pfId?"sel":""}>{f.data}</div>
      <div className={current?.pfId===f.pfId?"sel":""}>{money(f.base)}</div>
      <div className={current?.pfId===f.pfId?"sel":""}><select value={c.iva} onChange={e=>setp(f.pfId,"iva",+e.target.value)}><option value="21">21%</option><option value="10">10%</option><option value="0">0%</option></select></div>
      <div className={current?.pfId===f.pfId?"sel":""}><select value={c.ret} onChange={e=>setp(f.pfId,"ret",+e.target.value)}><option value="0">0%</option><option value="7">7%</option><option value="15">15%</option><option value="19">19%</option></select></div>
      <div className={current?.pfId===f.pfId?"sel":""}><select value={c.ded} onChange={e=>setp(f.pfId,"ded",+e.target.value)}>{Array.from({length:11}).map((_,i)=><option value={i*5}>{i*5}%</option>)}</select></div>
      <div className={current?.pfId===f.pfId?"sel":""}><b>{money(c.total)}</b></div>
      <div className={`fg-actions ${current?.pfId===f.pfId?"sel":""}`}><button onClick={()=>setSelected(f.pfId)}>Previsualitzar</button><button className="primary small" onClick={()=>openPrint(f)}>Imprimir</button></div>
    </React.Fragment>})}
  </div>
</Card>
...
function ProformaPrintV83({doc,pf}){
  const rows=pf.rows||[];
  const n=+pf.numeroCert||+(String(pf.numero||"").replace(/\D/g,""))||1;
  const baseCert=pf.base||0;
  const base=doc.base??baseCert;
  const iva=doc.iva||21, ret=doc.ret||0, ded=doc.ded||0;
  const ivaImp=doc.ivaImp??base*iva/100;
  const retImp=doc.retImp??base*ret/100;
  const total=doc.total??base+ivaImp-retImp;
  return <div className="proforma-print-v83">
    <h1>{doc.title}</h1><p className="doc-sub">{doc.subtitle}</p>
    <table><thead><tr><th>Codi</th><th>Concepte</th><th>Quant.</th><th>Preu</th><th>Import</th></tr></thead><tbody>{rows.map(r=>{let q=n===1?(+r.certAnterior||0):(+r.certActual||0);return <tr key={r.codi}><td>{r.codi}</td><td>{r.concepte}</td><td>{qty2(q)}</td><td>{money(r.pu)}</td><td>{money(q*r.pu)}</td></tr>})}</tbody></table>
    <div className="doc-totals v83"><div><span>Base certificada</span><b>{money(baseCert)}</b></div><div><span>Deducció {ded}%</span><b>-{money(baseCert-base)}</b></div><div><span>Base imposable</span><b>{money(base)}</b></div><div><span>IVA {iva}%</span><b>{money(ivaImp)}</b></div><div><span>Retenció {ret}%</span><b>-{money(retImp)}</b></div><div className="total"><span>Total proforma</span><b>{money(total)}</b></div></div>
  </div>
}
function CertPrintV83({doc}){
const rows=doc.rows||[];
const current=rows.filter(r=>(+r.qAct||0)>0);
const total=current.reduce((s,r)=>s+(+r.qAct||0)*(+r.pu||0),0);
return <div className="cert-print-v83">
<h1>{doc.title}</h1><p>{doc.subtitle}</p>
<h3>Resum de partides certificades</h3>
<table><thead><tr><th>Partida</th><th>Concepte</th><th>Q certificada</th><th>PU</th><th>Import</th></tr></thead><tbody>{current.map(r=><tr key={r.codi}><td>{r.codi}</td><td>{r.concepte}</td><td>{qty2(r.qAct)}</td><td>{money(r.pu)}</td><td>{money((+r.qAct||0)*(+r.pu||0))}</td></tr>)}</tbody><tfoot><tr><th colSpan="4">TOTAL</th><th>{money(total)}</th></tr></tfoot></table>
<div className="page-break-v83"></div>
<h2>Quadre resum</h2>
<div className="cert-grid-print-v83"><div className="h">Partida</div><div className="h">Ut</div><div className="h">Resum</div><div className="h">CanPres</div><div className="h">PrPres</div><div className="h">ImpPres</div><div className="h">Q ant.</div><div className="h">% ant.</div><div className="h">Imp ant.</div><div className="h">Q act.</div><div className="h">% act.</div><div className="h">Imp act.</div><div className="h">Total origen</div>{rows.map(r=><React.Fragment key={r.codi}><div>{r.codi}</div><div>{r.ut}</div><div className="concept">{r.concepte}</div><div>{qty2(r.q)}</div><div>{money(r.pu)}</div><div>{money((+r.q||0)*(+r.pu||0))}</div><div>{qty2(r.qPrev)}</div><div>{pct((+r.q||0)?(+r.qPrev||0)/(+r.q)*100:0)}</div><div>{money((+r.qPrev||0)*(+r.pu||0))}</div><div className={(+r.qAct||0)>0?"green":""}>{qty2(r.qAct)}</div><div className={(+r.qAct||0)>0?"green":""}>{pct((+r.q||0)?(+r.qAct||0)/(+r.q)*100:0)}</div><div className={(+r.qAct||0)>0?"green":""}>{money((+r.qAct||0)*(+r.pu||0))}</div><div>{money(((+r.qPrev||0)+(+r.qAct||0))*(+r.pu||0))}</div></React.Fragment>)}</div>
</div>
}
