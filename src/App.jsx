...
function tabsForWork8737(obra,data={}){
  ...
  if((data?.events||[]).length&&!tabs.includes("Agenda / Avisos")) tabs.splice(1,0,"Agenda / Avisos");
...
function Clients({clients,cs,setCs,ct,setCt,openClient,newClient}){
  const tipusOpts=["Promotor","Arquitecte","Arquitecte tècnic","Direcció Facultativa","Constructor","Constructora","Industrial","Administració","Particular","Autònom","Altres"];
  const total=clients.length;
  const empreses=clients.filter(c=>!["Particular"].includes(c.tipus)).length;
  return <div className="clients-page-v8773">
    <Card title="Clients del despatx" action={<button className="primary" onClick={newClient}><Plus/> Nou client</button>}>
      <div className="clients-hero-v8773"><div><h2>Base de dades de clients i agents</h2><p>Clients, promotors, constructors, industrials i tècnics vinculables als expedients.</p></div><div className="clients-kpis-v8773"><div><span>Total</span><b>{total}</b></div><div><span>Empreses / agents</span><b>{empreses}</b></div></div></div>
      <div className="filters filters-v8773"><div className="search-field"><Search size={16}/><input value={cs} onChange={e=>setCs(e.target.value)} placeholder="Buscar per nom, contacte, raó social..."/></div><select value={ct} onChange={e=>setCt(e.target.value)}><option value="">Totes les tipologies</option>{tipusOpts.map(t=><option key={t}>{t}</option>)}</select></div>
      <div className="client-grid-v8773">{clients.length===0?<Empty text="No hi ha clients amb aquest filtre."/>:clients.map(c=><button className="client-card-v8773" key={c.id} onClick={()=>openClient(c.id)}><div className={`client-logo ${c.color||"blue"}`}>{c.logo?<img src={c.logo}/>:(c.nom||"CL").slice(0,2).toUpperCase()}</div><div><strong>{c.nom}</strong><span>{c.rao}</span><small>{c.contacte||"Sense contacte"} · {c.telefon||"Sense telèfon"}</small><em>{[c.codiPostal,c.poblacio].filter(Boolean).join(" ")||c.adreca||"Sense població"}</em></div><b>{c.tipus||"Client"}</b></button>)}</div>
    </Card>
  </div>
}
...
function RentabilitatObra8773({data}){
  ...
  const certCalc=rows.reduce((s,r)=>s+certQtyTotal8773(r)*(+r.pu||0),0);
  const certificat=certGuardat||certCalc;
  const pendent=Math.max(pressupost-certificat,0);
  const desviacio=certificat-pressupost;
  const pctCert=pressupost?Math.min(140,certificat/pressupost*100):0;
  const pctDesv=pressupost?desviacio/pressupost*100:0;
  const topCaps=Object.entries(rows.reduce((m,r)=>{const k=r.cap||"Sense capítol";m[k]=(m[k]||0)+(+r.q||0)*(+r.pu||0);return m},{})).sort((a,b)=>b[1]-a[1]).slice(0,6);
  const topCert=Object.entries(rows.reduce((m,r)=>{const k=r.cap||"Sense capítol";m[k]=(m[k]||0)+certQtyTotal8773(r)*(+r.pu||0);return m},{})).sort((a,b)=>b[1]-a[1]).slice(0,6);
  return <div className="stack rentabilitat-v8773">
    <Card title="Rendibilitat i desviacions de l’obra"><div className="rent-kpis-v8773"><div><span>Pressupost obra</span><b>{money(pressupost)}</b></div><div><span>Certificat a origen</span><b>{money(certificat)}</b></div><div className={desviacio>0?"warn":"ok"}><span>Desviació prevista</span><b>{money(desviacio)}</b><small>{pctDesv.toFixed(2)}%</small></div><div><span>Pendent teòric</span><b>{money(pendent)}</b></div></div><div className="rent-chart-row-v8773"><div className="ring-card-v8773"><div className="ring-v8773" style={{background:`conic-gradient(#2563eb 0 ${pctCert*3.6}deg,#e5e7eb ${pctCert*3.6}deg 360deg)`}}><b>{pctCert.toFixed(1)}%</b></div><span>% certificat respecte pressupost</span></div><div className="bar-card-v8773"><b>Lectura ràpida</b><p>Compara el pressupost importat amb certificacions guardades o quantitats certificades. És un control intern orientatiu per detectar desviacions abans de facturar o validar amb DF.</p><div className="progress-v8773"><span style={{width:`${Math.min(100,pctCert)}%`}}/></div></div></div></Card>
    <div className="rent-two-v8773"><Card title="Pes del pressupost per capítol"><div className="chapter-bars-v8773">{topCaps.length===0?<Empty text="Encara no hi ha pressupost importat."/>:topCaps.map(([cap,val])=><div key={cap}><span>{cap}</span><em>{money(val)}</em><b style={{width:`${pressupost?Math.max(4,val/pressupost*100):0}%`}}/></div>)}</div></Card><Card title="Certificat per capítol"><div className="chapter-bars-v8773 green">{topCert.length===0?<Empty text="Encara no hi ha certificacions."/>:topCert.map(([cap,val])=><div key={cap}><span>{cap}</span><em>{money(val)}</em><b style={{width:`${certificat?Math.max(4,val/certificat*100):0}%`}}/></div>)}</div></Card></div>
  </div>
}
