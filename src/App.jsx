...
function LoginScreen8778({onLogin}){
  ...
  function submit(e){
    ...
    if(APP_USERS8779[u]&&APP_USERS8779[u]===pwd){try{if(window.PasswordCredential&&navigator.credentials&&navigator.credentials.store){navigator.credentials.store(new PasswordCredential({id:u,password:pwd,name:u})).catch(()=>{});}}catch(e){} sessionStorage.setItem("aco_current_user8779",u);onLogin?.(u);return}
  ...
  return <div className="login-page-v8778"><form className="login-card-v8778" onSubmit={submit} autoComplete="on" method="post" action="#login">
    ...
    <label><span>Usuari</span><input autoFocus name="username" id="aco-login-username" value={user} onChange={e=>setUser(e.target.value)} placeholder="Usuari" autoComplete="username"/></label>
    <label><span>Contrasenya</span><input type="password" name="password" id="aco-login-password" value={pwd} onChange={e=>setPwd(e.target.value)} placeholder="Contrasenya" autoComplete="current-password"/></label>
    {err&&<div className="login-error-v8778">{err}</div>}
    <button className="primary">Entrar</button><small>Les dades queden separades per usuari en aquest dispositiu. El navegador pot desar la contrasenya i, a iPad/mòbil, usar clauer/Face ID/Touch ID si està activat al dispositiu.</small>
...
let latest=[];
const sortTs=v=>{try{return new Date(v||0).getTime()||0}catch(e){return 0}};
latest.push(...events.map(e=>({tipus:e.type||"Agenda",ts:sortTs(e.updatedAt)||sortTs(`${e.year||""}-${String((e.month??0)+1).padStart(2,"0")}-${String(e.day||1).padStart(2,"0")}`),data:`${String(e.day||"--").padStart(2,"0")}/${String((e.month??0)+1).padStart(2,"0")}/${e.year||""}`,txt:e.title||e.note||"Cita / avís"})));
latest.push(...actes.map(a=>({tipus:"Acta",ts:sortTs(a.updatedAt)||sortTs(a.data),data:fmtAppDate8748(a.data),txt:a.titol||"Acta d’expedient"})));
latest.push(...pressupostos.map(p=>({tipus:"Pressupost",ts:sortTs(p.updatedAt)||sortTs(p.data),data:fmtAppDate8748(p.data),txt:p.concepte||p.nom||"Pressupost tècnic"})));
latest.push(...factures.map(f=>({tipus:"Factura",ts:sortTs(f.updatedAt)||sortTs(f.data),data:fmtAppDate8748(f.data),txt:f.numero||f.concepte||"Factura / proforma"})));
latest.push(...(data.certificacions||[]).map(c=>({tipus:"Certificació",ts:sortTs(c.updatedAt)||sortTs(c.data),data:fmtAppDate8748(c.data),txt:`Certificació ${c.numero||""} · ${money(c.import||0)}`})));
latest.push(...(data.partides||[]).filter(r=>r.updatedAt).slice(-12).map(r=>({tipus:"Pressupost obra",ts:sortTs(r.updatedAt),data:fmtAppDate8748(r.updatedAt),txt:`${r.codi||""} · ${r.concepte||"Partida modificada"}`})));
latest.push(...hores.map(h=>({tipus:"Temps",ts:sortTs(h.updatedAt)||sortTs(h.data),data:fmtAppDate8748(h.data),txt:`${h.tasca||h.etiqueta||"Temps registrat"} · ${(+h.hores||0).toFixed(2)} h`})));
latest=latest.sort((a,b)=>(b.ts||0)-(a.ts||0)).slice(0,8);
...
function certPrintHtmlV8772(doc,obra,client){
  ...
  return `<!doctype html><html><head><meta charset="utf-8"><title>${escHtmlV8772(doc.title||"Certificació")}</title><style>
    *{box-sizing:border-box} body{margin:0;background:white;color:#0f172a;font-family:Arial,Helvetica,sans-serif;font-size:10px}
    @page{size:A4 portrait;margin:12mm} @page certPortraitV8772{size:A4 portrait;margin:12mm} @page certLandscapeV8772{size:A4 landscape;margin:8mm}
    .page{background:#fff;break-after:page;page-break-after:always}.page:last-child{break-after:auto;page-break-after:auto}
    .portrait{page:certPortraitV8772;width:100%;max-width:190mm;min-height:270mm;padding:4mm}.landscape{page:certLandscapeV8772;width:100%;max-width:277mm;min-height:190mm;padding:3mm}
    ...
    @media print{body{background:#fff!important;padding:0!important}.page{box-shadow:none!important;margin:0 auto!important}.portrait{padding:2mm!important}.landscape{padding:1mm!important}}
  </style></head><body>
  ...
  return <Modal title={doc.title} close={close}>
    <div ref={printRef} className={`document-preview print-area ${doc.type==="certificacio"?"cert-doc-v8718":"portrait-doc"}`}>
      <div className="document-page modern-acta-page">
        {!["acta","certificacio","proforma"].includes(doc.type)&&<div className="cert-header-pro">
