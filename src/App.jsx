...
class SafeRenderBoundary878108 extends React.Component{
  ...
  render(){
    if(this.state.hasError){return <Card title="Pantalla protegida"><div className="module-note-v8738"><b>Aquesta pantalla s'ha protegit per evitar bloqueig.</b><span>{this.state.msg}</span></div><button className="primary" onClick={()=>this.setState({hasError:false,msg:""})}>Reintentar</button></Card>}
...
function normalizeAgendaEvent878108(e,i){
  const d=parseDateLoose8748(e?.data)||null;
  const day=Number(e?.day)||d?.getDate?.()||0;
  const month=(e?.month!==undefined&&e?.month!==null)?Number(e.month):(d?d.getMonth():-1);
  const year=Number(e?.year)||d?.getFullYear?.()||0;
  if(!day||month<0||!year)return null;
  return {...e,id:e.id||`evt-${year}-${month}-${day}-${i}`,day,month,year,title:e.title||e.titol||e.note||"Cita",hora:e.hora||""};
}
const safeIncomingEvents=Array.isArray(events)?events.map(normalizeAgendaEvent878108).filter(Boolean).slice(-300):[];
...
function load(){try{return (JSON.parse(localStorage.getItem(key)||"[]")||[]).map(normalizeAgendaEvent878108).filter(Boolean).slice(-300)}catch(e){return []}}
