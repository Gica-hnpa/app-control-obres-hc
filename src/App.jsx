...
function LoginScreen8778({onLogin}){
  ...
  function submit(e){
    e.preventDefault();
    if(user.trim().toLowerCase()==="hector" && pwd==="0000"){localStorage.setItem("aco_login_ok8778","1");onLogin?.();return}
    setErr("Usuari o contrasenya incorrectes. Prova: hector / 0000");
  }
  return <div className="login-page-v8778"><form className="login-card-v8778" onSubmit={submit}>
    <div className="login-logo-v8778">CO</div><h1>APP Control d’Obres</h1><p>Accés privat al mòdul tècnic del despatx.</p>
    <label><span>Usuari</span><input autoFocus value={user} onChange={e=>setUser(e.target.value)} placeholder="hector"/></label>
    <label><span>Contrasenya</span><input type="password" value={pwd} onChange={e=>setPwd(e.target.value)} placeholder="0000"/></label>
    {err&&<div className="login-error-v8778">{err}</div>}
    <button className="primary">Entrar</button><small>Usuari de prova: <b>hector</b> · Contrasenya: <b>0000</b></small>
  </form></div>
}
...
function DataJsonTools8778(){
  ...
  const keys=["aco_clients","aco_obres","aco_odata","aco_cp_custom8775","aco_config","aco_config_v60","aco_agenda_v86"];
  function exportJson(){
    const data={version:"V87.78",exportedAt:new Date().toISOString(),localStorage:{}};
    keys.forEach(k=>{data.localStorage[k]=localStorage.getItem(k)||null});
    const blob=new Blob([JSON.stringify(data,null,2)],{type:"application/json"});
    const url=URL.createObjectURL(blob);
    const a=document.createElement("a");a.href=url;a.download=`app-control-obres-backup-${new Date().toISOString().slice(0,10)}.json`;a.click();
    setTimeout(()=>URL.revokeObjectURL(url),1000);setStatus("Còpia JSON exportada.");
  }
  function importJson(file){
    ...
    reader.onload=()=>{
      try{
        ...
        keys.forEach(k=>{if(store[k]!==undefined&&store[k]!==null)localStorage.setItem(k,store[k])});
        setStatus("Dades importades. Recarregant l’app...");
...
const[authOk8778,setAuthOk8778]=useState(()=>localStorage.getItem("aco_login_ok8778")==="1");
