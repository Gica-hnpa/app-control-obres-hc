...
function ProformaPrintV81({doc,pf}){
  ...
  return <div className="proforma-print-v81">
    ...
    <p className="doc-sub">{doc.subtitle}</p>
    <h3>Partides certificades a origen</h3>
    <table className="proforma-lines-v87117"><colgroup><col className="c-code"/><col className="c-concept"/><col className="c-q"/><col className="c-price"/><col className="c-total"/></colgroup><thead><tr><th>Codi</th><th>Concepte / descripció</th><th>Q origen</th><th>Preu</th><th>Total origen</th></tr></thead><tbody>{rows.map(r=><tr key={r.codi}><td>{r.codi}</td><td className="concept"><b>{r.concepte}</b>{r.desc&&<details className="print-desc-toggle-v87117"><summary>Veure descripció</summary><small>{r.desc}</small></details>}</td><td className="num">{qty2(r.qOrigin??0)}</td><td className="num">{money(r.pu)}</td><td className="num">{money(r.impOrigin??0)}</td></tr>)}</tbody><tfoot><tr><th colSpan="4">TOTAL A ORIGEN</th><th className="num">{money(totalOrigen)}</th></tr></tfoot></table>
    <h3>Deducció de certificacions anteriors</h3>
...
function certPrintHtmlV8772(doc,obra,client){
  ...
  const originRowsHtml=originRows.map(r=>`<tr><td>${escHtmlV8772(r.codi)}</td><td>${escHtmlV8772(r.ut)}</td><td class="concept"><b>${escHtmlV8772(r.concepte)}</b>${r.desc?`<details class="print-desc-toggle-v87117"><summary>Veure descripció</summary><small>${escHtmlV8772(r.desc)}</small></details>`:""}</td><td class="num">${qty2(r.qOrigin)}</td><td class="num">${money(r.pu)}</td><td class="num">${money(r.impOrigin)}</td></tr>`).join("") || `<tr><td colspan="6" class="empty">No hi ha partides certificades a origen.</td></tr>`;
  let lastCap="__none__";
  const wideRows=rows.map(r=>{
    ...
    return `${showCap?`<tr class="cap"><td colspan="15">${escHtmlV8772(cap)}</td></tr>`:""}<tr>
      <td>${escHtmlV8772(r.codi)}</td><td>${escHtmlV8772(r.ut)}</td><td class="concept"><b>${escHtmlV8772(r.concepte)}</b>${r.desc?`<details class="print-desc-toggle-v87117"><summary>Veure descripció</summary><small>${escHtmlV8772(r.desc)}</small></details>`:""}</td><td>${qty2(r.q)}</td><td>${money(r.pu)}</td><td>${money(pres)}</td>
