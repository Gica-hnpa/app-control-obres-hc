...
function statusKeyPress8776(estat){const e=String(estat||"Pendent").toLowerCase();if(e.includes("accept"))return"acceptats";if(e.includes("no")||e.includes("descart")||e.includes("rebutj"))return"no acceptats";if(e.includes("fet")||e.includes("enviat")||e.includes("emès")||e.includes("emes"))return"fets";return"pendents"}
function statusKeyFactura8776(estat){const e=String(estat||"Pendent").toLowerCase();if(e.includes("cobrat")||e.includes("cobrada"))return"cobrades";if(e.includes("pendent")||e.includes("no cob")||e.includes("impagat"))return"no cobrades";return"fetes"}
...
function aggregate8776(rows,keyFn,valueFn){const map={};rows.forEach(r=>{const k=keyFn(r)||"Sense classificar";map[k]=(map[k]||0)+(valueFn?valueFn(r):1)});return Object.entries(map).map(([k,v])=>({k,v})).sort((a,b)=>b.v-a.v)}
...
function Donut8776({title,parts,total,kind="money"}){const colors=["#2563eb","#22c55e","#f59e0b","#ef4444","#8b5cf6","#06b6d4"];return <div className="dash-donut-card-v8776"><h3>{title}</h3><div className="dash-donut-v8776" style={donutStyle8776(parts,colors)}><span>{kind==="count"?total:money(total)}</span></div><div className="dash-donut-legend-v8776">{parts.map((p,i)=><div key={p.k}><i style={{background:colors[i%colors.length]}}></i><b>{p.k}</b><span>{kind==="count"?p.v:money(p.v)}</span></div>)}</div></div>}
...
useEffect(()=>{
  ...
  try{
    ...
    c=sanitizeClients8785(rawC,(isHector?clients0:[])).map(x=>x.id==="socoterm"?{...x,logo:x.logo||SOCOTERM_LOGO}:x);
    ...
    c=isHector?sanitizeClients8785(clients0,[]):[];
...
{screen==="Agenda"&&<Agenda events={[...Object.entries(odata).flatMap(([oid,d])=>(d.events||[]).map(e=>({...e,obraId:e.obraId||oid,client:e.client||clients.find(c=>c.id===obres.find(o=>o.id===oid)?.client)?.nom,obra:e.obra||obres.find(o=>o.id===oid)?.nom,adreca:e.adreca||obres.find(o=>o.id===oid)?.adreca}))),...invoiceAlerts8776(obres,odata)]} clients={clients} obres={obres} openObra={openObra} openEvent={()=>setModal("event")} calM={calM} setCalM={setCalM} calY={calY} setCalY={setCalY} selDay={selDay} setSelDay={setSelDay} setOdata={setOdata}/>}
...
function FitxaDadesTab8769({obra,client,save,allAgents=[],setData,openAgent}){
  ...
  function addMissingAgents(){
    if(!setData)return;
    const roles={constructor:"Constructor / contractista",do:"Direcció d’obra",deo:"Direcció d’execució d’obra",css:"Coordinació de seguretat i salut"};
    setData(d=>{
      let arr=[...(d.agents||[])];
      Object.entries(roles).forEach(([key,rol])=>{
        const nom=String(form[key]||"").trim();
        if(!nom||nom==="Pendent")return;
        if(!arr.some(a=>normCity8773(a.nom)===normCity8773(nom))){arr.push({id:"ag-fitxa-"+key+"-"+Date.now()+Math.random().toString(16).slice(2),nom,rol,empresa:nom,email:"",telefon:""})}
      });
...
<Card title={`CERTIFICACIÓ ${certNum} ACTUAL · ${prevNum?`Cert. ${prevNum} anterior + `:"sense anterior + "}Cert. ${certNum}`}>
  ...
  <div className="cert-toolbar-v69">
    ...
    <div className="actions-inline">
      ...
      <button className="secondary" onClick={()=>openDoc({type:"certificacio",title:`CERTIFICACIÓ ${certNum}`,subtitle:`Import: ${money(certTotal(certNum))}`,certNum,prevNum,includeMesures:includeMesures8780,rows:rows.map(r=>({...r,qPrev:qFor(r,prevNum),qAct:qDraft(r),qOrigen:qOrigin(r),impOrigen:qOrigin(r)*(+r.pu||0),pctOrigen:pc(qOrigin(r),r),mesures:(r.certMesuresByNum||{})[String(certNum)]||[]})),totalActual:certTotal(certNum),totalOrigen:totalOrigin(),data:fmtDate8714(cert?.data)})}>Previsualitzar</button>
    </div>
  </div>
...
function set(k,v){setForm(f=>({...f,[k]:v}))}
function clearForm(){setForm({title:"",client:clientFilter||"",clientNou:"",promotor:"",obra:obraFilter||"",obraNova:"",tipus:"Visita d’obra",hora:"09:00",adreca:"",detail:"",obraId:""})}
function editNote(n){setSelDay(+n.day||selDay);setCalM(+n.month||calM);setCalY(+n.year||calY);setForm({id:n.id,obraId:n.obraId||"",title:n.title||"",client:n.client||"",clientNou:"",promotor:n.promotor||"",obra:n.obra||"",obraNova:"",tipus:n.tipus||n.type||"Nota",hora:n.hora||"09:00",adreca:n.adreca||"",detail:n.detail||n.note||""})}
function updateExpEvent(note){
  if(!note.obraId||!setOdata)return false;
  setOdata(prev=>{const d=prev[note.obraId]||{};const list=d.events||[];const exists=list.some(x=>x.id===note.id);return {...prev,[note.obraId]:{...d,events:exists?list.map(x=>x.id===note.id?note:x):[...list,note]}}});
  return true;
}
function saveNote(){
 ...
 if(existsLocal){saveList(local.map(n=>n.id===note.id?note:n));}
 else{saveList([...local,note]);}
 if(obraId){updateExpEvent(note);}
 setForm({...note,clientNou:"",obraNova:""});
}
function deleteNote(){
 if(!form.id)return;
 if(!confirm("Eliminar aquesta cita o avís?"))return;
 if(local.some(n=>n.id===form.id))saveList(local.filter(n=>n.id!==form.id));
 if(form.obraId&&setOdata){setOdata(prev=>{const d=prev[form.obraId]||{};return {...prev,[form.obraId]:{...d,events:(d.events||[]).filter(x=>x.id!==form.id)}}});setVersion(v=>v+1)}
 clearForm()
}
...
<Card title="Agenda / Calendari">
 ...
 {Array.from({length:total}).map((_,i)=>{let day=i+1;let ev=filtered.filter(e=>e.day===day&&e.month===calM&&e.year===calY);return <button key={day} onClick={()=>setSelDay(day)} className={`day ${selDay===day?"active":""} ${day===todayDay&&calM===todayMonth&&calY===todayYear?"today":""}`}><b>{day}</b>{ev.slice(0,3).map(e=><small key={e.id} onClick={(evn)=>{evn.stopPropagation();editNote(e)}}>{e.hora?e.hora+" · ":""}{e.client?e.client+" · ":""}{e.title}</small>)}{ev.length>3&&<em>+{ev.length-3}</em>}</button>})}</div>
</Card>
<Card title={`Nota / visita · ${selectedDate}`} action={<div className="actions-inline"><button className="secondary" onClick={clearForm}>Nova neta</button>{form.id&&<button className="danger" onClick={deleteNote}>Eliminar</button>}<button className="primary" onClick={saveNote}>Guardar cita / canvis</button></div>}>
 ...
 <h3>Notes del dia</h3>
 <div className="notes-list-v86 agenda-notes-v8749">{selected.length===0?<p className="muted">No hi ha notes per aquest dia.</p>:selected.map(n=><button key={n.id} className={`note-card-v86 ${form.id===n.id?"active":""}`} onClick={()=>editNote(n)} onDoubleClick={()=>{if(n.obraId&&openObra)openObra(n.obraId)}}><b>{n.title}</b><span>{String(n.day).padStart(2,"0")}/{String(n.month+1).padStart(2,"0")}/{n.year} · {n.client||"Sense client"} · {n.obra||"Sense expedient"}</span><small>{n.tipus||n.type||"Nota"} · {n.hora||""} {n.adreca?`· ${n.adreca}`:""}</small><p>{n.detail||n.note||""}</p>{n.obraId&&<em>Doble clic per obrir expedient</em>}</button>)}</div>{form.id&&<div className="agenda-detail-v8749"><b>Detall de l’avís seleccionat</b><span><strong>Avís:</strong> {form.title}</span><span><strong>Client:</strong> {form.client||"—"}</span><span><strong>Expedient:</strong> {form.obra||"—"}</span><span><strong>Hora:</strong> {form.hora||"—"}</span><span><strong>Adreça:</strong> {form.adreca||"—"}</span><p>{form.detail||"Sense observacions."}</p>{(() => { const found=all.find(e=>e.id===form.id); return found?.obraId&&openObra?<button className="primary" onClick={()=>openObra(found.obraId)}>Obrir expedient</button>:null })()}</div>}
</Card>
</div>
...
function HonorarisGeneral({obres,odata,setOdata,openObra,openObraTab}){
  ...
  function updatePressupostGlobal(row,patch){
    ...
    setOdata(prev=>{
      ...
      return {...prev,[row.obra.id]:{...d,pressupostosTecnic:(d.pressupostosTecnic||[]).map(p=>p.id===row.id?{...p,...patch}:p)}};
    });
...
function ProformaPrintV81({doc,pf}){
  ...
  return <div className="proforma-print-v81">
    ...
    <h3>Deducció de certificacions anteriors</h3>
    <table className="totals-preview"><tbody><tr><th>Total certificat a origen</th><td className="num">{money(totalOrigen)}</td></tr>{prevTotals.length===0?<tr><th>No hi ha certificacions anteriors</th><td className="num">{money(0)}</td></tr>:prevTotals.map(c=><tr key={c.n}><th>Deducció Certificació {c.n}</th><td className="num">-{money(c.total)}</td></tr>)}<tr className="total"><th>Base certificada actual / Cert. {certNum}</th><td className="num">{money(base)}</td></tr></tbody></table>
