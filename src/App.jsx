...
function save(){
  ...
  Object.assign(client,form);setEdit(false);alert("Client guardat. Refresca si el llistat no s’actualitza al moment.")
}
...
<div className="client-edit-head-v72"><div className="client-logo big">{form.logo?<img src={form.logo}/>:"LOGO"}</div>{edit&&<label className="upload-label secondary">Canviar logo<input type="file" accept="image/*" onChange={e=>f2u(e.target.files[0],u=>set("logo",u))}/></label>}</div>
<div className="form-grid">
<Input label="Nom comercial" value={form.nom||""} onChange={e=>set("nom",e.target.value)} disabled={!edit}/><Input label="Raó social" value={form.rao||""} onChange={e=>set("rao",e.target.value)} disabled={!edit}/><Input label="NIF/CIF" value={form.nif||""} onChange={e=>set("nif",e.target.value)} disabled={!edit}/><Input label="Contacte" value={form.contacte||""} onChange={e=>set("contacte",e.target.value)} disabled={!edit}/><Input label="Email" value={form.email||""} onChange={e=>set("email",e.target.value)} disabled={!edit}/><Input label="Telèfon" value={form.telefon||""} onChange={e=>set("telefon",e.target.value)} disabled={!edit}/>
</div></Card><Card title={`Obres amb ${form.nom||client.nom}`}><div className="list">{obres.map(o=><ObraRow key={o.id} o={o} open={openObra}/>)}</div></Card></div>}
...
const key="aco_fact_params_v72";
const[params,setParams]=useState(()=>{try{return JSON.parse(localStorage.getItem(key)||"{}")}catch(e){return {}}});
const[selected,setSelected]=useState(null);
useEffect(()=>{try{localStorage.setItem(key,JSON.stringify(params))}catch(e){}},[params]);
...
function rowsForCert(c){let n=+c.numero;return (data.partides||[]).filter(r=>n===1?(+r.certAnterior>0):(+r.certActual>0))}
...
return <div className="fact-layout-v72">
<Card title="Factures proforma de certificacions">
  <div className="fact-table-v72">
    <div className="fact-head-v72"><span>Proforma</span><span>Data</span><span>Base</span><span>IVA</span><span>Ret.</span><span>Ded.</span><span>Total</span><span>Accions</span></div>
    {proformes.map(f=>{let c=calc(f);return <div className={`fact-row-v72 ${current?.pfId===f.pfId?"selected":""}`} key={f.pfId}>
      <span><b>{f.numero}</b><small>Cert. {f.numeroCert}</small></span><span>{f.data}</span><span>{money(f.base)}</span>
      <span><select value={c.iva} onChange={e=>setp(f.pfId,"iva",+e.target.value)}><option value="21">21%</option><option value="10">10%</option><option value="0">0%</option></select></span>
      <span><select value={c.ret} onChange={e=>setp(f.pfId,"ret",+e.target.value)}><option value="0">0%</option><option value="7">7%</option><option value="15">15%</option><option value="19">19%</option></select></span>
      <span><select value={c.ded} onChange={e=>setp(f.pfId,"ded",+e.target.value)}>{Array.from({length:11}).map((_,i)=><option value={i*5}>{i*5}%</option>)}</select></span>
      <span><b>{money(c.total)}</b></span><span className="row-actions"><button onClick={()=>setSelected(f.pfId)}>Previsualitzar</button><button className="primary small" onClick={()=>openPrint(f)}>Imprimir</button></span>
    </div>})}
  </div>
</Card>
...
function DocViewer({doc,obra,client,close,email}){let acta=doc.acta,agents=doc.agents||[],pf=doc.proforma,actaPhotos=doc.actaPhotos||[],actaDocs=doc.actaDocs||[];let assistents=acta?acta.agents.map(id=>agents.find(a=>a.id===id)).filter(Boolean):[];return <Modal title={doc.title} close={close}><div className={`document-preview print-area ${doc.type==="certificacio"?"landscape-doc":"portrait-doc"}`}><div className="document-page modern-acta-page"><div className="cert-header-pro"><div>{(client.logo||SOCOTERM_LOGO)?<img className="doc-logo" src={client.logo||SOCOTERM_LOGO}/>:<div className="fake-logo">LOGO</div>}<h3>{client.rao}</h3><p>NIF: {client.nif}<br/>Adreça: {client.adreca||"Pendent"}<br/>{client.email}<br/>{client.telefon}</p></div><div><h3>{obra.propietat}</h3><p>NIF: {obra.nifPropietat}<br/>{obra.adreca}<br/>{obra.poblacio}</p></div></div>{doc.type==="certificacio"&&doc.rows?<div className="certificacio-doc-v72">
<h1>{doc.title}</h1><p>{doc.subtitle}</p>
<table><thead><tr><th>Partida</th><th>Concepte</th><th>Q actual</th><th>PU</th><th>Import actual</th></tr></thead><tbody>{doc.rows.filter(r=>(+r.qAct||0)>0).map(r=><tr><td>{r.codi}</td><td>{r.concepte}</td><td>{qty2(r.qAct)}</td><td>{money(r.pu)}</td><td>{money((+r.qAct||0)*(+r.pu||0))}</td></tr>)}</tbody></table>
<div className="page-break-v72"></div>
<h2>Quadre resum de certificació</h2>
<div className="client-cert-grid-v72">
