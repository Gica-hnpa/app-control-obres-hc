...
function AgendaExpedient8774({data,setData,obra,client}){
  ...
  function save(){const ev=cleanAgendaEvent87109({...form,id:form.id||`ev-${Date.now()}`,obraId:obra?.id,obra:obra?.nom,client:client?.nom||obra?.propietat||"",data:form.data});if(!ev)return;setData(d=>({...d,events:(Array.isArray(d.events)?d.events:[]).some(x=>String(x.id)===String(ev.id))?d.events.map(x=>String(x.id)===String(ev.id)?ev:x):[...(Array.isArray(d.events)?d.events:[]),ev],updatedAt:new Date().toISOString()}));setForm({id:"",data:today.toISOString().slice(0,10),hora:"09:00",title:"",tipus:"Visita d’obra",adreca:obra?.adreca||"",detail:""})}
