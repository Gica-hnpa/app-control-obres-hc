...
function dedupePartidaLibrary87196(rows=[]){
  ...
  let seq=items.reduce((m,x)=>Math.max(m,+String(x.codiIntern||"").match(/(\d+)$/)?.[1]||0),0);
  items.forEach(x=>{if(!x.codiIntern)x.codiIntern=`LIB-${String(++seq).padStart(6,"0")}`});
...
function PartidesLibraryGeneral87196({items=[],setItems,clients=[]}){
  ...
  const filtered=scoped.filter(x=>{
    ...
    return okQ&&(!capFilter||String(x.cap||"General")===capFilter);
  });
  ...
  return <div className="library-page-v87196 stack">
    ...
    <Card title="Consultar i treballar la llibreria" action={<button type="button" className="secondary" onClick={consolidate}>Revisar duplicats</button>}>
      ...
      <div className="library-list-v87196">{filtered.length===0?<Empty text={scope==="client"?"Aquest client encara no té partides. Pots afegir-les des d'un pressupost o des de la llibreria global.":"Encara no hi ha partides globals."}/>:filtered.slice(0,160).map(item=>{
        ...
        return <details className="library-item-v87196" key={item.id}><summary><div><b>{item.concepte}</b><span>{item.codiIntern} · {item.cap||"General"} · {item.ut||"ut"}</span><small>{item.global?"Global":"Només clients"}{owners.length?` · ${owners.join(", ")}`:""}</small></div><div><small>Preu actual</small><strong>{money(item.pu||0)}</strong><em>Editar ▾</em></div></summary><div className="library-editor-grid-v87196"><label><span>Codi intern</span><input value={item.codiIntern||""} readOnly/></label><label><span>Codi visible orientatiu</span><input defaultValue={item.codiPressupost||item.codi||""} onBlur={e=>updateItem(item.id,{codiPressupost:e.target.value,codi:e.target.value})}/></label><label className="span-all"><span>Concepte</span><input defaultValue={item.concepte||""} onBlur={e=>updateItem(item.id,{concepte:e.target.value})}/></label><label><span>Unitat</span><input defaultValue={item.ut||"ut"} onBlur={e=>updateItem(item.id,{ut:e.target.value})}/></label><label className="library-chapter-select-v87198"><span>Canviar de capítol</span><select value={item.cap||"General"} onChange={e=>changeItemChapter87198(item,e.target.value)}>{allChapterStats.map(ch=><option key={ch.cap} value={ch.cap}>{ch.cap}</option>)}<option value="__new__">+ Crear un capítol nou...</option></select>{chapterSavedId===item.id&&<small>Capítol canviat i guardat</small>}</label><label><span>Preu actual</span><input inputMode="decimal" defaultValue={qty2(item.pu||0)} onBlur={e=>updateItem(item.id,{pu:parseNum8770(e.target.value)||0})}/></label><label className="span-all"><span>Descripció llarga</span><textarea defaultValue={item.desc||""} onBlur={e=>updateItem(item.id,{desc:e.target.value})}/></label><label className="library-check-v87196"><input type="checkbox" checked={!!item.global} onChange={e=>updateItem(item.id,{global:e.target.checked})}/><span>Disponible globalment</span></label><details className="library-client-links-v87196"><summary>Clients vinculats ({(item.clientIds||[]).length})</summary>{clients.map(c=><label key={c.id}><input type="checkbox" checked={(item.clientIds||[]).some(id=>String(id)===String(c.id))} onChange={e=>updateItem(item.id,{clientIds:e.target.checked?[...new Set([...(item.clientIds||[]),String(c.id)])]:(item.clientIds||[]).filter(id=>String(id)!==String(c.id))})}/><span>{c.nom||c.rao}</span></label>)}</details>{(item.priceHistory||[]).length>0&&<details className="library-price-history-v87196"><summary>Històric de preus ({item.priceHistory.length})</summary>{item.priceHistory.slice().reverse().map((p,i)=><span key={`${p.data}-${p.pu}-${i}`}>{p.data||"—"} · {money(p.pu||0)} · {p.origen||"Llibreria"}</span>)}</details>}</div><div className="library-item-actions-v87196"><button type="button" className="danger" onClick={()=>removeItem(item.id)}>Eliminar de la llibreria</button></div></details>
      })}</div>
    </Card>
  </div>
}
...
function Pressupost({data,setData,importExcel,deletePressupostVersion,duplicatePressupostVersion,openPartida,openEmail,openDoc,client,obra,clientHistoricalPartides=[],budgetGroups=[],activeBudgetId="principal",selectBudget,addBudget,totalGlobal=0,totalActive=0,partidaLibrary=[],setPartidaLibrary}){
  ...
  const clientLibraryKey87115=`aco_partides_library_history_v87196_${libraryClientToken87196(client?.id||client?.nom||client?.rao||"general")}`;
  ...
  useEffect(()=>{
    ...
    const rows=(clientHistoricalPartides||[]).filter(r=>r&&String(r.concepte||"").trim()).map(r=>({...r,id:undefined,codiIntern:"",codiPressupost:r.codi||"",clientIds:currentClientId87196?[currentClientId87196]:[],global:false,tipus:`Històric ${r.sourceObra||"client"}`,cap:r.cap||"General",updatedAt:new Date().toISOString()}));
    if(!rows.length)return;
    setPartidaLibrary?.(prev=>dedupePartidaLibrary87196([...(prev||[]),...rows]));
    lsSet8779(seedKey,historicalSeedSig87116);
  },[historicalSeedSig87116,clientLibraryKey87115,currentClientId87196]);
  ...
  function seedLibraryFromBudget87115(){
    const flat=Object.entries(caps||{}).flatMap(([cap,items])=>(items||[]).map(r=>({...r,id:r.libraryItemId||undefined,codiIntern:r.codiIntern||"",codiPressupost:r.codi||"",cap,clientIds:currentClientId87196?[currentClientId87196]:[],global:false,tipus:"Llibreria client",updatedAt:new Date().toISOString()})));
    if(!flat.length)return alert("No hi ha partides per actualitzar la llibreria.");
    setPartidaLibrary?.(prev=>dedupePartidaLibrary87196([...(prev||[]),...flat]));
    setLibraryOpen87115(true);
    alert(`${flat.length} partida/es revisades. Les coincidents s'han unificat.`);
  }
