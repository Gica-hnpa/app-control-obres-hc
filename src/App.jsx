...
export default 
function downloadBackupFile8732(){
  try{
    const payload={
      app:"Control d'Obres",
      version:"87.32",
    ...
    alert("No s’ha pogut desar la còpia JSON: "+String(err?.message||err));
  }
}
function importBackupFile8732(file){
  if(!file)return;
  ...
  reader.onload=()=>{
    try{
      ...
      if(!keys.length)throw new Error("El fitxer no conté dades reconegudes de l’app");
...
function BackupFilePanel8732(){
  const inputRef=useRef(null);
  return <Card title="Còpia de seguretat i traspàs de dades">
    <div className="backup-file-v8732">
      <div>
        <b>Treballar entre PC i iPad</b>
        <span>Desa un fitxer JSON al PC, guarda’l a Drive/iCloud i importa’l després a l’iPad. No s’executa res automàticament.</span>
      </div>
      <div className="actions-inline backup-actions-v8732">
        <button type="button" className="primary" onClick={()=>downloadBackupFile8732()}>Desar còpia JSON</button>
        <button type="button" className="secondary" onClick={()=>inputRef.current?.click()}>Importar còpia JSON</button>
        <input ref={inputRef} type="file" accept="application/json,.json" style={{display:"none"}} onChange={e=>importBackupFile8732(e.target.files?.[0])}/>
      </div>
    </div>
  </Card>
}
