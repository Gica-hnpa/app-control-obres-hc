...
function allAgents8749(odata={},clients=[]){
  ...
  const clientAgents=(clients||[]).map(c=>({id:"client-agent-"+c.id,nom:c.contacte&&c.contacte!=="Pendent"?c.contacte:c.nom,rol:c.tipus||"Client",empresa:c.rao||c.nom,email:c.email||"Pendent",telefon:c.telefon||"",nif:c.nif||"",adreca:c.adreca||""}));
  return uniqAgents8749([...base,...clientAgents,...Object.values(odata||{}).flatMap(d=>d.agents||[])]);
}
function fallbackExtract8749(wb){
  ...
  for(const sn of wb.SheetNames){
    ...
    for(const r of rows){
      ...
      if((!E&&!F&&!G) && (rowText.includes("capitol") || rowText.includes("cap ") || (/^\d{1,3}([.\- ]\d+)?$/.test(A)&&C) || (A&&C&&!isCode(A)))){
...
function Pressupost({data,importExcel,deletePressupostVersion,duplicatePressupostVersion,openPartida,openEmail,openDoc}){
  const [caps,setCaps]=useState(()=>group(activePartides8750(data),"cap"));
  const [open,setOpen]=useState(()=>Object.fromEntries(Object.keys(group(activePartides8750(data),"cap")).map((k,i)=>[k,i===0])));
  const [descOpen875,setDescOpen875]=useState({});
  useEffect(()=>{const syncCapsV874=group(activePartides8750(data),"cap");setCaps(syncCapsV874);setOpen(Object.fromEntries(Object.keys(syncCapsV874).map((k,i)=>[k,i===0])));},[data.partides,data.pressupostos]);
...
const[rows,setRows]=useState(()=>{try{return JSON.parse(localStorage.getItem(key)||"null")||[]}catch(e){return []}});
const[form,setForm]=useState({data:new Date().toISOString().slice(0,10),tipus:"Visita obra",tasca:"",hores:"1.00",preu:"50.00",facturat:"No",entregat:"Pendent",cobrat:"Pendent"});
useEffect(()=>{localStorage.setItem(key,JSON.stringify(rows))},[rows,key]);
function add(){const h=Number(String(form.hores).replace(",","."))||0, p=Number(String(form.preu).replace(",","."))||0;if(!form.tasca.trim()){alert("Indica la tasca feta.");return}setRows(r=>[{id:"hr-"+Date.now(),...form,hores:h,preu:p},...r]);setForm({...form,tasca:"",hores:"1.00"})}
function upd(id,k,v){setRows(r=>r.map(x=>x.id===id?{...x,[k]:v}:x))}
function del(id){if(confirm("Eliminar aquest registre de temps?"))setRows(r=>r.filter(x=>x.id!==id))}
const totalH=rows.reduce((s,x)=>s+(+x.hores||0),0);
const total=rows.reduce((s,x)=>s+(+x.hores||0)*(+x.preu||0),0);
const facturat=rows.filter(x=>x.facturat==="Sí").reduce((s,x)=>s+(+x.hores||0)*(+x.preu||0),0);
const pendent=total-facturat;
return <div className="stack temps-safe-v8750"><Card title="Gestió del temps de l’expedient"><div className="honor-kpis"><Kpi t="HORES" v={`${totalH.toFixed(2)} h`}/><Kpi t="VALOR INTERN" v={money(total)}/><Kpi t="FACTURAT" v={money(facturat)}/><Kpi t="PENDENT" v={money(pendent)}/></div></Card><Card title="Afegir registre de temps"><div className="form-grid"><label><span>Data</span><input type="date" value={form.data} onChange={e=>setForm({...form,data:e.target.value})}/></label><label><span>Tipus feina</span><select value={form.tipus} onChange={e=>setForm({...form,tipus:e.target.value})}><option>Visita obra</option><option>Acta</option><option>Pressupost</option><option>Certificació</option><option>Redacció informe</option><option>Trucada / gestió</option><option>Administració</option><option>Altres</option></select></label><label><span>Tasca</span><input value={form.tasca} onChange={e=>setForm({...form,tasca:e.target.value})}/></label><label><span>Hores</span><input type="number" step="0.01" value={form.hores} onChange={e=>setForm({...form,hores:e.target.value})}/></label><label><span>€/h</span><input type="number" step="0.01" value={form.preu} onChange={e=>setForm({...form,preu:e.target.value})}/></label></div><div className="card-actions"><button className="primary" onClick={add}>Afegir registre</button></div></Card><Card title="Registres"><div className="time-table-wrap"><table className="time-table"><thead><tr><th>Data</th><th>Tipus</th><th>Tasca</th><th>Hores</th><th>€/h</th><th>Import</th><th>Facturat</th><th>Entregat</th><th>Cobrat</th><th></th></tr></thead><tbody>{rows.length===0&&<tr><td colSpan="10"><Empty text="Encara no hi ha registres de temps."/></td></tr>}{rows.map(r=><tr key={r.id}><td><input type="date" value={r.data||""} onChange={e=>upd(r.id,"data",e.target.value)}/></td><td><input value={r.tipus||""} onChange={e=>upd(r.id,"tipus",e.target.value)}/></td><td><input value={r.tasca||""} onChange={e=>upd(r.id,"tasca",e.target.value)}/></td><td><input type="number" step="0.01" value={r.hores} onChange={e=>upd(r.id,"hores",e.target.value)}/></td><td><input type="number" step="0.01" value={r.preu} onChange={e=>upd(r.id,"preu",e.target.value)}/></td><td><b>{money((+r.hores||0)*(+r.preu||0))}</b></td><td><select value={r.facturat||"No"} onChange={e=>upd(r.id,"facturat",e.target.value)}><option>No</option><option>Sí</option></select></td><td><select value={r.entregat||"Pendent"} onChange={e=>upd(r.id,"entregat",e.target.value)}><option>Pendent</option><option>Entregat</option></select></td><td><select value={r.cobrat||"Pendent"} onChange={e=>upd(r.id,"cobrat",e.target.value)}><option>Pendent</option><option>Cobrat</option></select></td><td><button className="danger" onClick={()=>del(r.id)}>Eliminar</button></td></tr>)}</tbody></table></div></Card></div>
}
