...
function deletePressupostVersion(id){
  ...
  setD(obraId,d=>{
    ...
    return {...d,pressupostos:next,partides:next.length?d.partides:[],certificacions:next.length?d.certificacions:[]};
  });
}
...
function Actes({data,allAgents:globalAgents=[],openActa,openEmail,openDoc,selected,setSelected}){
const baseAgent={id:"ag-hector",nom:"Héctor Cubero",rol:"Tècnic",empresa:"Despatx tècnic",email:"pendent"};
const agents=ensureAgents8748(uniqAgents8749([baseAgent,...(globalAgents||[]),...(data.agents||[])]));
const actes=data.actes||[];
const active=actes.find(a=>a.id===selected)||actes[0]||null;
return <div className="actes-stable-v8750b"><Card title="Actes creades" action={<button type="button" className="primary" onClick={openActa}><Plus/> Nova acta</button>}>
  <div className="acta-list stable">{actes.length===0?<Empty text="Encara no hi ha actes creades."/>:actes.map(a=><button key={a.id} className={`acta-list-row ${active?.id===a.id?"active":""}`} onClick={()=>setSelected?.(a.id)}><strong>{a.titol||"Acta"}</strong><span>{a.data||"Sense data"}</span><small>{(a.agents||[]).map(id=>agents.find(x=>x.id===id)?.nom).filter(Boolean).join(", ")||"Sense agents seleccionats"}</small></button>)}</div>
</Card>
<Card title={active?`Previsualització · ${active.titol}`:"Previsualització"} action={active&&<div className="actions-inline"><button type="button" className="secondary" onClick={()=>openDoc?.({type:"acta",title:active.titol,subtitle:active.data,acta:active,agents})}>Obrir document</button><button type="button" className="secondary" onClick={()=>openEmail?.(active.titol)}><Mail/> Enviar</button></div>}>
  {!active?<Empty text="Selecciona o crea una acta."/>:<div className="acta-preview-stable-v8750b"><div className="form-grid"><label><span>Títol</span><input value={active.titol||""} readOnly/></label><label><span>Data</span><input value={active.data||""} readOnly/></label><label className="span-all"><span>Agents disponibles / assistents</span><div className="check-grid">{agents.map(ag=><label className="check-row" key={ag.id}><input type="checkbox" checked={(active.agents||[]).includes(ag.id)} readOnly/><span>{ag.nom} · {ag.rol} · {ag.empresa}</span></label>)}</div></label><label className="span-all"><span>Text acta</span><textarea value={active.text||""} readOnly/></label></div><div className="a4-mini-v8750b"><h3>ACTA DE VISITA / SEGUIMENT</h3><b>{active.titol}</b><p><b>Data:</b> {active.data}</p><p>{active.text||"Text pendent"}</p></div></div>}
</Card></div>}
...
const key=`aco_temps_expedient_${obraId||"default"}`;
...
function add(){setRows(p=>[{id:"tm-"+Date.now(),data:new Date().toISOString().slice(0,10),tasca:"Treball tècnic",hores:1,facturat:false,entregat:false,cobrat:false,obs:""},...p])}
...
return <Card title="Gestió temps" action={<button type="button" className="primary" onClick={add}>+ Afegir temps</button>}>
  <div className="temps-stable-v8750b"><div className="honor-kpis"><Kpi t="REGISTRES" v={rows.length}/><Kpi t="HORES TOTALS" v={total.toFixed(2)+" h"}/></div><div className="table-scroll-v8750b"><table className="simple-table-v8750b"><thead><tr><th>Data</th><th>Feina</th><th>Hores</th><th>Facturat</th><th>Entregat</th><th>Cobrat</th><th>Observacions</th><th></th></tr></thead><tbody>{rows.length===0&&<tr><td colSpan="8">Encara no hi ha temps registrat.</td></tr>}{rows.map(r=><tr key={r.id}><td><input type="date" value={r.data||""} onChange={e=>upd(r.id,"data",e.target.value)}/></td><td><input value={r.tasca||""} onChange={e=>upd(r.id,"tasca",e.target.value)}/></td><td><input type="number" step="0.25" value={r.hores||0} onChange={e=>upd(r.id,"hores",e.target.value)}/></td><td><input type="checkbox" checked={!!r.facturat} onChange={e=>upd(r.id,"facturat",e.target.checked)}/></td><td><input type="checkbox" checked={!!r.entregat} onChange={e=>upd(r.id,"entregat",e.target.checked)}/></td><td><input type="checkbox" checked={!!r.cobrat} onChange={e=>upd(r.id,"cobrat",e.target.checked)}/></td><td><input value={r.obs||""} onChange={e=>upd(r.id,"obs",e.target.value)}/></td><td><button className="danger small" onClick={()=>del(r.id)}>Eliminar</button></td></tr>)}</tbody></table></div></div>
</Card>}
...
const[tipus,setTipus]=useState(WORK_TYPES8737[0]);
const[selectedFinal,setSelectedFinal]=useState("__same__");
const main=clients.find(c=>c.id===clientSel);
const final=selectedFinal==="__same__"?main:clients.find(c=>c.id===selectedFinal);
return <form onSubmit={onSubmit} className="form-stable-v8750b"><div className="form-grid">
  <label><span>Client del treball *</span><select name="client" value={clientSel} onChange={e=>setClientSel(e.target.value)} required><option value="__new__">+ Crear client nou</option><option value="" disabled>— Clients existents —</option>{clients.map(c=><option key={c.id} value={c.id}>{c.nom}</option>)}</select></label>
  {clientSel==="__new__"&&<><Input name="clientNouNom" label="Nom nou client *" defaultValue="Nou client"/><Input name="clientNouRao" label="Raó social" defaultValue="Pendent"/><label><span>Tipologia</span><select name="clientNouTipus"><option>Particular</option><option>Promotor</option><option>Arquitecte tècnic</option><option>Constructor</option><option>Industrial</option><option>Administració</option><option>Altres</option></select></label><Input name="clientNouContacte" label="Contacte" defaultValue="Pendent"/><Input name="clientNouNif" label="NIF/CIF" defaultValue="Pendent"/><Input name="clientNouTelefon" label="Telèfon" defaultValue="Pendent"/><Input name="clientNouEmail" label="Email" defaultValue="Pendent"/><Input name="clientNouAdreca" label="Adreça" defaultValue="Pendent"/></>}
  <label><span>Client final / propietat</span><select value={selectedFinal} onChange={e=>setSelectedFinal(e.target.value)}><option value="__same__">Mateix que el client del treball</option><option value="__newfinal__">+ Escriure client final nou</option>{clients.map(c=><option key={c.id} value={c.id}>{c.nom}</option>)}</select></label>
  <Input name="nom" label="Nom de l’expedient *" defaultValue="Nou expedient"/>
  <Input name="subtitol" label="Descripció breu" defaultValue="Treball pendent de definir"/>
  <label><span>Tipus de treball *</span><select name="tipusTreball" value={tipus} onChange={e=>setTipus(e.target.value)} required>{WORK_TYPES8737.map(t=><option key={t}>{t}</option>)}</select></label>
  {tipus==="Altres"&&<Input name="tipusTreballAltres" label="Especifica el tipus de treball" defaultValue=""/>}
  <label><span>Paraula clau del codi</span><input name="paraulaClau" placeholder="Ex: FRONTMAR, GARRIGOLES, PALAMOS"/></label>
  <Input name="any" label="Any obertura" defaultValue="2026"/>
  <label><span>Estat</span><select name="estat"><option>Pressupostada</option><option>Acceptada</option><option>Activa</option><option>En procés</option><option>Tancada</option></select></label>
  {selectedFinal==="__newfinal__"? <><Input name="propietat" label="Nom client final / propietat" defaultValue="Pendent"/><Input name="nifPropietat" label="NIF client final" defaultValue="Pendent"/></> : <><Input name="propietat" label="Client final / propietat" defaultValue={final?.nom||"Pendent"}/><Input name="nifPropietat" label="NIF client final" defaultValue={final?.nif||"Pendent"}/></>}
  <Input name="adreca" label="Adreça completa *" defaultValue={final?.adreca||"Pendent"}/>
  <Input name="poblacio" label="Població" defaultValue="Pendent"/>
  <Input name="rc" label="Referència cadastral" defaultValue="Pendent"/>
